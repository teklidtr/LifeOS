...
def _task_snapshot(raw_task: dict[str, Any] | None, raw_event: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "task_title": str(
            raw_event.get("task_title") or task.get("title") or raw_event.get("task_id") or ""
        ),
        "task_shape": str(
            raw_event.get("task_shape")
            or task.get("task_shape")
            or task.get("template")
            or "unspecified"
        ),
        ...
        "blocked": bool(raw_event.get("blocked", bool(task.get("blocked_by"))))
        if raw_event.get("blocked") is not None or task
        else None,
...
def _canonical_event(
    *,
    ...
    if not isinstance(event_id, str) or not event_id.strip():
        diagnostics.append(
            FeedbackDiagnostic(
                "invalid_event_id", "Execution event requires a non-empty event_id.", source_path
            )
        )
    ...
    if type(schema) is not int or schema != _SUPPORTED_EVENT_SCHEMA:
        diagnostics.append(
            FeedbackDiagnostic(
                ...
                f"Execution event {event_id} uses unsupported schema {schema!r}.",
                source_path,
                event_id,
            )
        )
    ...
    if not isinstance(task_id, str) or not task_id.strip():
        diagnostics.append(
            FeedbackDiagnostic(
                "invalid_task_id",
                f"Execution event {event_id} requires a task_id.",
                source_path,
                event_id,
            )
        )
    ...
    if not isinstance(outcome, str) or outcome not in _OUTCOMES:
        diagnostics.append(
            FeedbackDiagnostic(
                "invalid_outcome",
                f"Execution event {event_id} has an invalid outcome.",
                source_path,
                event_id,
            )
        )
    ...
    if day is None:
        diagnostics.append(
            FeedbackDiagnostic(
                "invalid_event_date",
                f"Execution event {event_id} requires an ISO date.",
                source_path,
                event_id,
            )
        )
    ...
    planned, planned_ok = _optional_int(
        {"planned_minutes": snapshot["planned_minutes"]}, "planned_minutes", low=0, high=1440
    )
    ...
    if not planned_ok or not actual_ok:
        diagnostics.append(
            FeedbackDiagnostic(
                "invalid_duration",
                f"Execution event {event_id} has an impossible duration.",
                source_path,
                event_id,
            )
        )
    ...
    if not fraction_ok:
        diagnostics.append(
            FeedbackDiagnostic(
                ...
                f"Execution event {event_id} has an invalid completion fraction.",
                source_path,
                event_id,
            )
        )
    ...
    for field in ("energy_before", "energy_after", "motivation_before"):
        ...
        if value is not None and value not in _LEVELS:
            diagnostics.append(
                FeedbackDiagnostic(
                    "invalid_capacity_level",
                    f"Execution event {event_id} has invalid {field}.",
                    source_path,
                    event_id,
                )
            )
    ...
    if raw.get("started_at") and started is None or raw.get("ended_at") and ended is None:
        diagnostics.append(
            FeedbackDiagnostic(
                "invalid_chronology",
                f"Execution event {event_id} has an invalid timestamp.",
                source_path,
                event_id,
            )
        )
        ...
        try:
            if ended < started:
                diagnostics.append(
                    FeedbackDiagnostic(
                        "invalid_chronology",
                        f"Execution event {event_id} ends before it starts.",
                        source_path,
                        event_id,
                    )
                )
            ...
            diagnostics.append(
                FeedbackDiagnostic(
                    "invalid_chronology",
                    f"Execution event {event_id} mixes timezone-aware and naive timestamps.",
                    source_path,
                    event_id,
                )
            )
    ...
    return {
        ...
        if isinstance(raw.get("corrects_event_id"), str)
        else None,
        "retracts_event_id": raw.get("retracts_event_id")
        if isinstance(raw.get("retracts_event_id"), str)
        else None,
    }, tuple(diagnostics)


def build_evidence_dataset(
    vault_root: Path, *, as_of: date | None = None, excluded_event_ids: tuple[str, ...] = ()
) -> EvidenceDataset:
    diagnostics: list[FeedbackDiagnostic] = []
    ...
    try:
        ...
        return EvidenceDataset(
            ...
            "",
            (),
            (FeedbackDiagnostic("storage_unavailable", str(exc)),),
            (),
            0,
            0,
        )
        ...
        for index, raw in enumerate(history):
            if not isinstance(raw, dict):
                diagnostics.append(
                    FeedbackDiagnostic(
                        ...
                        "Execution event must be a mapping.",
                        source.relative_path,
                    )
                )
                continue
            ...
            if event is not None:
                if raw_task is None:
                    diagnostics.append(
                        FeedbackDiagnostic(
                            "orphaned_task_reference",
                            f"Execution event {event['event_id']} refers to a task no longer present in the plan.",
                            source.relative_path,
                            event["event_id"],
                            "warning",
                        )
    ...
    for event_id in sorted(duplicate_ids):
        diagnostics.append(
            FeedbackDiagnostic(
                "duplicate_event_id", f"Duplicate execution event ID: {event_id}", event_id=event_id
            )
        )
    ...
    for event_id, event in by_id.items():
        ...
        if retracts:
            if retracts in retractions:
                diagnostics.append(
                    FeedbackDiagnostic(
                        "conflicting_retractions",
                        f"Multiple events retract {retracts}.",
                        event_id=event_id,
                    )
                )
        ...
        if len(children) > 1:
            ...
            diagnostics.append(
                FeedbackDiagnostic(
                    ...
                    f"Multiple corrections target {target}: {', '.join(sorted(children))}.",
                    event_id=target,
                )
            )
    ...
    for event_id, event in sorted(by_id.items()):
        if (
            event_id in conflicted
            or event_id in retracted_ids
            or event_id in replaced_ids
            or event.get("retracts_event_id")
        ):
            ...
            if parent in seen_lineage or parent not in by_id:
                diagnostics.append(
                    FeedbackDiagnostic(
                        ...
                        f"Correction {event_id} has missing or cyclic lineage.",
                        event_id=event_id,
                    )
                )
                lineage = []
        ...
        observation_id = _stable_hash(
            event["source_path"], event_id, event["source_hash"], str(event["source_index"])
        )
        observations.append(
            FeedbackObservation(
    ...
    diagnostics.sort(
        key=lambda item: (item.source_path or "", item.event_id or "", item.code, item.message)
    )
...
def serialize_dataset(dataset: EvidenceDataset) -> str:
    return (
        json.dumps(dataset.to_dict(), sort_keys=True, default=str, ensure_ascii=False, indent=2)
        + "\n"
    )


def rebuild_evidence_dataset(
    vault_root: Path,
    runtime_dir: Path,
    *,
    as_of: date | None = None,
    excluded_event_ids: tuple[str, ...] = (),
) -> tuple[EvidenceDataset, EvidenceDatasetStatus]:
    ...
    try:
        ...
        status = EvidenceDatasetStatus(
            ...
            len(dataset.diagnostics),
            len(dataset.excluded_event_ids),
            dataset.corrected_event_count,
            dataset.retracted_event_count,
            dataset.source_fingerprint,
            str(target),
            False,
        )
        return dataset, status
    state: Literal["empty", "diagnostic", "ready"] = (
        "empty"
        if not dataset.observations and not dataset.diagnostics
        else "diagnostic"
        if any(item.severity == "error" for item in dataset.diagnostics)
        else "ready"
    )
    return dataset, EvidenceDatasetStatus(
        state,
        DATASET_SCHEMA_VERSION,
        len(dataset.observations),
        len(dataset.diagnostics),
