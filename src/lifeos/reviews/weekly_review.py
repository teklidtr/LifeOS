...
_PROMPTS = (
    ...
    WeeklyReviewPrompt("weekly-pattern", "Which repeated pattern is supported by explicit evidence?"),
...
def weekly_due_state(artifact: ReviewArtifact, now: datetime) -> WeeklyReviewDueState:
    ...
    if phase.state == "completed" or artifact.metadata.status == "completed":
        return WeeklyReviewDueState("completed", "This weekly review is completed and may be reopened.")
    if phase.state == "skipped" or artifact.metadata.status == "skipped":
        return WeeklyReviewDueState("skipped", "This weekly review was intentionally skipped and may be reopened.")
    if now.date() > artifact.metadata.period_end:
        return WeeklyReviewDueState("overdue", "The review period has ended and the artifact remains open.")
    if now.date() == artifact.metadata.period_end and now.timetz().replace(tzinfo=None) >= time(17, 0):
...
def open_weekly_review(
    *,
    ...
    artifact = service.open_or_create(kind="weekly", day=day, timezone=timezone, now=now, idempotency_key=f"{idempotency_key}-open")
    if refresh:
...
def complete_weekly_review(
    *,
    ...
    artifact = progress.update_phase(review_id=state.artifact.metadata.review_id, phase_id="weekly", action="complete", required_sections=state.required_sections, expected_hash=state.artifact.content_hash, idempotency_key=f"{idempotency_key}-phase", now=now)
    artifact = progress.complete_review(review_id=artifact.metadata.review_id, expected_hash=artifact.content_hash, idempotency_key=f"{idempotency_key}-review", now=now)
    return WeeklyReviewState(artifact, state.snapshot, state.prompts, state.required_sections, weekly_due_state(artifact, now), None)
