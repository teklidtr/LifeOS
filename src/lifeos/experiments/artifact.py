...
def _render_managed(metadata: ExperimentMetadata) -> str:
    ...
    if protocol.outcome_measures:
        for measure in protocol.outcome_measures:
            ...
            lines.append(f"- `{measure.measure_id}` {measure.display_name}{unit}: {measure.kind}, {measure.role}, {measure.cadence}")
    else:
        lines.append("- No measures defined.")
    lines.extend(["", "## Phases", ""])
    for phase in protocol.phases:
        lines.append(f"- `{phase.phase_id}` {phase.name}: {phase.start_date} to {phase.end_date} ({phase.kind})")
    ...
    for observation in metadata.observations[-30:]:
        ...
        lines.append(f"- {observation.observed_at} · `{observation.measure_id}` · `{observation.phase_id}` · {rendered}")
    ...
    for amendment in metadata.amendments:
        lines.append(f"- {amendment.created_at} · **{amendment.reason}**: {'; '.join(amendment.changes)}")
    if not metadata.amendments:
        lines.append("- No protocol amendments.")
    if metadata.analyses:
        latest = metadata.analyses[-1]
        lines.extend(["", "## Latest analysis", "", f"- Status: `{latest.status}`", f"- Evidence: `{latest.evidence_kind}`"])
        for summary in latest.summaries:
            lines.append(f"- {summary.get('label', summary.get('measure_id', 'Summary'))}: {summary.get('text', summary)}")
...
def parse_experiment(path: Path, relative_path: str, content: str) -> ExperimentArtifact:
    ...
    if len(matches) != 1:
        raise ExperimentError("malformed_artifact", "The managed experiment block must appear exactly once.")
    match = matches[0]
    human_body = (parsed.body[: match.start()] + parsed.body[match.end() :]).strip("\n") + "\n"
    metadata = metadata_from_dict(frontmatter)
    return ExperimentArtifact(relative_path, "sha256:" + content_hash(content), metadata, human_body)
...
class ExperimentArtifactService:
    ...
    def create(
        ...
        *,
        ...
        initial = LifecycleEvent(f"life-{secrets.token_hex(6)}", None, "idea", moment.isoformat(), "created")
    ...
    def list(self, *, states: frozenset[str] | None = None) -> tuple[ExperimentArtifact, ...]:
        ...
        artifacts = tuple(parse_experiment(item.path, item.relative_path, item.content) for item in sources)
        selected = artifacts if states is None else tuple(item for item in artifacts if item.metadata.state in states)
        return tuple(sorted(selected, key=lambda item: (item.metadata.updated_at, item.metadata.experiment_id), reverse=True))
    ...
    def save(self, artifact: ExperimentArtifact, metadata: ExperimentMetadata, *, expected_hash: str) -> ExperimentArtifact:
        current = self.load(artifact.path)
        if current.content_hash != expected_hash:
            raise ExperimentError("stale_artifact", "Experiment changed after it was opened.", {"actual_hash": current.content_hash})
        _atomic_write(
            self.vault_root,
            artifact.path,
    ...
    def transition(
        ...
        *,
        ...
        if target in {"baseline", "scheduled", "active"} and not artifact.metadata.safety.allows_activation:
            raise ExperimentError(
                "unsafe_experiment",
        ...
        event = LifecycleEvent(
            f"life-{secrets.token_hex(6)}", artifact.metadata.state, target, moment.isoformat(), reason.strip()
        )
        metadata = replace(
            artifact.metadata, state=target, updated_at=moment.isoformat(), lifecycle=(*artifact.metadata.lifecycle, event)
        )
    ...
    def amend_protocol(
        ...
        *,
        ...
        if artifact.metadata.state in {"idea", "drafting"}:
            raise ExperimentError("amendment_not_required", "Edit the draft protocol directly before baseline begins.")
        moment = utc_now(now)
        amendment = ProtocolAmendment(
            f"amend-{secrets.token_hex(6)}", moment.isoformat(), reason.strip(), changes, protocol_hash(artifact.metadata.protocol)
        )
        metadata = replace(
            artifact.metadata,
            protocol=protocol,
    ...
    def append_observation(
        ...
        *,
        ...
        artifact = self.load(relative_path)
        if any(item.observation_id == observation.observation_id for item in artifact.metadata.observations):
    ...
    def clone(self, relative_path: str, *, title: str | None = None, now: datetime | None = None) -> ExperimentArtifact:
        ...
        return self.create(
            ...
            now=now,
            parent_experiment_id=original.metadata.parent_experiment_id or original.metadata.experiment_id,
