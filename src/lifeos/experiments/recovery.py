    ...
    *,
    ...
    for source in sources:
        ...
        if identity in artifacts:
            ...
            diagnostics.append({
                "code": "duplicate_identity", "experiment_id": identity,
                "paths": [getattr(earlier, "path"), artifact.path],
            })
            continue
    ...
    for artifact in artifacts.values():
        references = [
            ...
            *(ref for observation in artifact.metadata.observations for ref in observation.source_refs),
        ]
        for ref in references:
            if not (vault_root / ref.path).exists():
                diagnostics.append({
                    "code": "missing_linked_source", "experiment_id": artifact.metadata.experiment_id,
                    "experiment_path": artifact.path, "source_path": ref.path, "relation": ref.relation,
                })
    ...
    for identity, old in previous_by_id.items():
        ...
        if current is None:
            diagnostics.append({"code": "deleted_artifact", "experiment_id": identity, "previous_path": old.path})
        elif current.path != old.path:
            diagnostics.append({"code": "moved_artifact", "experiment_id": identity, "previous_path": old.path, "current_path": current.path})
    ...
    for source in observation_sources:
        ...
        if not identity or identity not in artifacts:
            diagnostics.append({"code": "orphaned_observation", "path": source.relative_path, "experiment_id": identity or None})
    index = rebuild_experiment_index(
        vault_root=vault_root, runtime_dir=runtime_dir, interrupt_after=interrupt_after
    ) if rebuild else previous
    state = "interrupted" if index.state == "interrupted" else "attention" if diagnostics else index.state
