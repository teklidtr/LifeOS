...
def _numeric(observations: Iterable[Observation]) -> tuple[tuple[str, float], ...]:
    ...
    for item in observations:
        ...
        if item.state != "measured" or not isinstance(raw_value, (int, float, bool)):
            continue
        value = 1.0 if raw_value is True else 0.0 if raw_value is False else float(raw_value)
...
def analyze_experiment(
    artifact: ExperimentArtifact, *, now: datetime | None = None
) -> AnalysisRecord:
    moment = utc_now(now)
    ...
    for measure in metadata.protocol.outcome_measures:
        measure_observations = tuple(
            item for item in observations if item.measure_id == measure.measure_id
        )
        ...
        phase_summaries = {
            phase_id: _summary(values) for phase_id, values in sorted(phase_values.items())
        }
        baseline_values = tuple(
            pair
            for phase_id, values in phase_values.items()
            if phases_by_kind.get(phase_id) == "baseline"
            for pair in values
        )
        intervention_values = tuple(
            pair
            for phase_id, values in phase_values.items()
            if phases_by_kind.get(phase_id) == "intervention"
            for pair in values
        )
        ...
        if isinstance(baseline_mean, (int, float)) and isinstance(intervention_mean, (int, float)):
            change = float(intervention_mean) - float(baseline_mean)
        summaries.append(
            {
                "measure_id": measure.measure_id,
                "label": measure.display_name,
                ...
                "text": (
                    ...
                    else "Not enough measured baseline and intervention values for a comparison."
                ),
    ...
    if adherence:
        ...
        adherence_observations = tuple(
            item for item in observations if item.measure_id in adherence_ids
        )
        ...
        summaries.append(
            {
                "label": "Adherence",
                "text": f"Observed adherence rate: {rate}."
                if rate is not None
                else "No measured adherence data.",
                "rate": rate,
            }
        )
    measured_count = sum(item.state == "measured" for item in observations)
    primary_ids = {
        item.measure_id for item in metadata.protocol.outcome_measures if item.role == "primary"
    }
    baseline_primary = sum(
        item.state == "measured"
        and item.measure_id in primary_ids
        and phases_by_kind.get(item.phase_id) == "baseline"
        for item in observations
    )
    intervention_primary = sum(
        item.state == "measured"
        and item.measure_id in primary_ids
        and phases_by_kind.get(item.phase_id) == "intervention"
        for item in observations
    )
    ...
    if metadata.safety.level in {"blocked", "emergency"}:
        ...
        limitations.append(
            "At least two measured primary observations in both baseline and intervention are required for a phase comparison."
        )
    elif any(
        "confound" in item.casefold()
    ...
    return AnalysisRecord(
        f"analysis-{secrets.token_hex(8)}",
        moment.isoformat(),
        status,
        tuple(summaries),
...
def save_analysis(
    ...
    *,
    ...
    metadata = replace(
        artifact.metadata,
        analyses=(*artifact.metadata.analyses, analysis),
...
def record_conclusion(
    ...
    *,
    ...
    if not artifact.metadata.analyses and conclusion not in {
