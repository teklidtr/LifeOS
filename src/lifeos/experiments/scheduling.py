...
def _timezone(name: str) -> ZoneInfo:
    try:
        ...
        raise ExperimentError("invalid_timezone", "Experiment schedule timezone is unknown.", {"timezone": name}) from exc


def _days_for_measure(measure: MeasureDefinition, start: date, end: date, selected_days: tuple[int, ...]) -> tuple[date, ...]:
    result: list[date] = []
    current = start
    while current <= end:
...
def build_collection_windows(metadata: ExperimentMetadata, *, now: datetime) -> tuple[CollectionWindow, ...]:
    schedule: Mapping[str, object] = metadata.protocol.schedule
    timezone_name = str(schedule.get("timezone", "UTC"))
    tz = _timezone(timezone_name)
    if now.tzinfo is None:
        raise ExperimentError("invalid_timestamp", "Schedule evaluation requires a timezone-aware timestamp.")
    local_now = now.astimezone(tz)
    due_time = time.fromisoformat(str(schedule.get("time", "20:00")))
    window_minutes = int(schedule.get("window_minutes", 120))
    grace_minutes = int(schedule.get("grace_minutes", 60))
    selected_days = tuple(int(item) for item in schedule.get("weekdays", (0, 1, 2, 3, 4, 5, 6)))
        ...
        for measure in metadata.protocol.outcome_measures:
            for day in _days_for_measure(measure, start, end, selected_days):
                ...
                if metadata.state == "paused":
                    ...
                    status = "overdue"
                windows.append(CollectionWindow(due.isoformat(), opens.isoformat(), closes.isoformat(), measure.measure_id, phase.phase_id, status))
...
def due_windows(metadata: ExperimentMetadata, *, now: datetime) -> tuple[CollectionWindow, ...]:
    recorded = {(item.measure_id, item.phase_id, item.observed_at[:10]) for item in metadata.observations}
    return tuple(
        window for window in build_collection_windows(metadata, now=now)
        if window.status in {"open", "overdue"} and (window.measure_id, window.phase_id, window.due_at[:10]) not in recorded
    )
