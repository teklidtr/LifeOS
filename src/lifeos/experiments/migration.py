...
def _load_audit(runtime_dir: Path) -> dict[str, dict[str, str]]:
    ...
    try:
        ...
        return {str(key): {str(k): str(v) for k, v in dict(value).items()} for key, value in dict(entries).items()}
    except (OSError, json.JSONDecodeError, TypeError, ValueError) as exc:
        raise ExperimentError("migration_audit_invalid", "Experiment migration audit is malformed.", {"error": str(exc)}) from exc
...
def _legacy(frontmatter: Mapping[str, Any]) -> bool:
    return frontmatter.get("type") in {"experiment", "personal-experiment-v0"} or frontmatter.get("experiment_schema") == 0


def _stable_timestamp(frontmatter: Mapping[str, Any]) -> datetime:
    raw = frontmatter.get("created_at") or frontmatter.get("start_date")
    if not raw:
        raise ExperimentError("legacy_date_missing", "Legacy experiment requires created_at or start_date for stable migration identity.")
    text = str(raw)
    try:
        ...
        try:
            ...
            raise ExperimentError("legacy_date_invalid", "Legacy experiment date is invalid.", {"value": text}) from exc
...
def _metadata(source_path: str, source_hash: str, frontmatter: Mapping[str, Any]) -> ExperimentMetadata:
    ...
    state = state_raw if state_raw in {"idea", "drafting", "baseline", "scheduled", "active", "paused", "completed", "abandoned", "analyzed", "archived"} else "idea"
    created_at = moment.isoformat()
    lifecycle = (LifecycleEvent(f"life-migration-{suffix}", None, state, created_at, f"migrated from {source_path}"),)
...
def _target_path(metadata: ExperimentMetadata) -> str:
    slug = "-".join(part for part in "".join(char if char.isalnum() else " " for char in metadata.title.casefold()).split())[:64] or "experiment"
    timestamp = metadata.experiment_id.split("-", 2)[1]
    return f"experiments/{timestamp[:4]}/{slug}-{metadata.experiment_id}.md"


def preview_experiment_migration(*, vault_root: Path, runtime_dir: Path) -> ExperimentMigrationPreview:
    ...
    for source in sources:
        ...
        source_info = LegacyExperimentSource(
            source.relative_path, source_hash, str(parsed.frontmatter.get("title", source.path.stem)),
        ...
        if diagnostics:
            candidates.append(ExperimentMigrationCandidate(source_info, None, None, "malformed", diagnostics, None))
            continue
        try:
            ...
            candidates.append(ExperimentMigrationCandidate(source_info, None, None, "malformed", (message,), None))
            continue
        audited = audit.get(source.relative_path)
        state: MigrationState = "ready"
        if audited:
            if audited.get("source_hash") == source_hash and (vault_root / audited.get("target_path", "")).exists():
                state = "already-migrated"
                target_path = audited["target_path"]
            else:
                state = "conflict"
                diagnostics = ("Migration audit exists but the source hash or canonical target no longer matches.",)
        elif (vault_root / target_path).exists():
            state = "conflict"
            diagnostics = ("Canonical migration target already exists without a matching audit record.",)
        candidates.append(ExperimentMigrationCandidate(
            source_info, target_path, metadata.experiment_id, state, diagnostics, metadata.to_frontmatter()
        ))
    ...
    *,
    ...
    for candidate in preview.candidates:
        ...
        if expected != candidate.source.content_hash:
            conflicts.append(ExperimentMigrationCandidate(
                candidate.source, candidate.target_path, candidate.experiment_id, "conflict",
                (*candidate.diagnostics, "Legacy source changed after preview or was not explicitly approved."),
        ...
        if content_hash(source.content) != candidate.source.content_hash:
            conflicts.append(ExperimentMigrationCandidate(
                candidate.source, candidate.target_path, candidate.experiment_id, "conflict",
                (*candidate.diagnostics, "Legacy source changed during migration."), candidate.planned_frontmatter,
            ))
            continue
        from .contracts import metadata_from_dict
        metadata = metadata_from_dict(candidate.planned_frontmatter)
        human_body = "## Imported legacy note\n\n" + source.content.strip() + "\n\n## User annotations\n\n"
        ...
        if interrupt_after is not None and processed >= interrupt_after:
            return ExperimentMigrationResult("interrupted", tuple(migrated), tuple(already), tuple(conflicts), preserved, str(_audit_path(runtime_dir)))
    state = "conflict" if conflicts else "ready"
    return ExperimentMigrationResult(state, tuple(migrated), tuple(already), tuple(conflicts), preserved, str(_audit_path(runtime_dir)))
