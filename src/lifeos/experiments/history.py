...
def rebuild_experiment_index(
    *,
    ...
    if batch_size < 1:
        raise ExperimentError("invalid_batch_size", "Experiment rebuild batch size must be positive.")
    ...
    for source in sources:
        try:
            ...
            diagnostics.append({"code": exc.code, "path": source.relative_path, "message": exc.message})
            continue
        identity = artifact.metadata.experiment_id
        if identity in seen:
            diagnostics.append({"code": "duplicate_identity", "experiment_id": identity, "paths": [seen[identity], artifact.path]})
            continue
        seen[identity] = artifact.path
        entries.append(ExperimentIndexEntry(
            identity, artifact.path, artifact.content_hash, artifact.metadata.title, artifact.metadata.state,
            artifact.metadata.category, artifact.metadata.updated_at, artifact.metadata.conclusion,
            artifact.metadata.parent_experiment_id, artifact.metadata.repeated_from_experiment_id,
            tuple(item.measure_id for item in artifact.metadata.protocol.outcome_measures),
        ))
        processed += 1
        if processed % batch_size == 0:
            checkpoint.write_text(json.dumps({"processed": processed, "last_path": artifact.path}, sort_keys=True) + "\n")
        if interrupt_after is not None and processed >= interrupt_after:
            checkpoint.write_text(json.dumps({"processed": processed, "last_path": artifact.path, "interrupted": True}, sort_keys=True) + "\n")
            return ExperimentIndexReport("interrupted", tuple(entries), tuple(diagnostics), str(checkpoint))
    entries.sort(key=lambda item: (item.updated_at, item.experiment_id), reverse=True)
    payload = {"schema": 1, "entries": [item.to_dict() for item in entries], "diagnostics": diagnostics}
...
def load_experiment_index(*, runtime_dir: Path) -> ExperimentIndexReport:
    ...
    if not path.exists():
        return ExperimentIndexReport("missing-index", (), ({"code": "rebuild_required", "message": "Experiment index is missing."},))
    try:
        data = json.loads(path.read_text())
        entries = tuple(ExperimentIndexEntry(
            str(item["experiment_id"]), str(item["path"]), str(item["content_hash"]), str(item["title"]),
            str(item["state"]), str(item["category"]), str(item["updated_at"]),
            ...
            str(item["repeated_from_experiment_id"]) if item.get("repeated_from_experiment_id") else None,
            tuple(str(value) for value in item.get("measure_ids", ())),
        ) for item in data.get("entries", ()))
        return ExperimentIndexReport("ready", entries, tuple(dict(item) for item in data.get("diagnostics", ())))
    except (OSError, json.JSONDecodeError, KeyError, TypeError) as exc:
        return ExperimentIndexReport("corrupt-index", (), ({"code": "rebuild_required", "message": str(exc)},))


def compare_experiments(entries: Iterable[ExperimentIndexEntry], left_id: str, right_id: str) -> dict[str, object]:
    ...
    return {
        "left": left.to_dict(), "right": right.to_dict(), "compatible": compatible,
        "warning": None if compatible else "The experiments do not share a measure identity; results must not be combined.",
    }
