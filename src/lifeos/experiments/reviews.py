...
def _source(artifact: ExperimentArtifact, detail: str, generated_at: str) -> tuple[object, ...]:
...
def daily_experiment_section(
    *, vault_root: Path, runtime_dir: Path, day: date, generated_at: datetime
    ...
    try:
        ...
        return ReviewSectionSnapshot("experiments-daily", "Experiments today", True, "unavailable", (), exc.message)
    for artifact in artifacts:
        metadata = artifact.metadata
        windows = tuple(window for window in due_windows(metadata, now=generated_at) if window.due_at[:10] == day.isoformat())
        stop_warning = metadata.safety.level in {"caution", "blocked", "emergency"}
        amendment_pending = bool(metadata.amendments and metadata.amendments[-1].created_at[:10] == day.isoformat())
        ...
        fingerprint = stable_fingerprint(
            artifact.path, artifact.content_hash, day, *(f"{window.measure_id}:{window.phase_id}:{window.status}" for window in windows),
            metadata.safety.level, metadata.amendments[-1].amendment_id if amendment_pending else "",
        )
        items.append(ReviewItemSnapshot(
            f"experiment-day:{metadata.experiment_id}", "experiments-daily", metadata.title, detail,
            fingerprint, "ready", "record-observation", _source(artifact, detail, generated_at.isoformat()),
        ))
...
def weekly_experiment_section(
    *, vault_root: Path, runtime_dir: Path, range_start: date, range_end: date, generated_at: datetime
) -> "ReviewSectionSnapshot":
    from lifeos.reviews.contracts import ReviewItemSnapshot, ReviewSectionSnapshot, ReviewSourceReference, stable_fingerprint
    ...
    try:
        ...
        return ReviewSectionSnapshot("experiments-weekly", "Experiment review", True, "unavailable", (), exc.message)
    for artifact in artifacts:
        metadata = artifact.metadata
        recently_updated = range_start.isoformat() <= metadata.updated_at[:10] <= range_end.isoformat()
        ...
        amendments = sum(range_start.isoformat() <= item.created_at[:10] <= range_end.isoformat() for item in metadata.amendments)
        ...
        items.append(ReviewItemSnapshot(
            f"experiment-week:{metadata.experiment_id}", "experiments-weekly", metadata.title, detail,
            stable_fingerprint(artifact.path, artifact.content_hash, detail), "ready", action,
