...
def _frontmatter_rows(
    vault_root: Path, roots: tuple[str, ...], *, note_type: str | None = None
...
def _journal(vault_root: Path, day: date) -> DashboardSection:
    ...
    try:
        ...
        if exc.code == "not-found":
            return DashboardSection(
                "empty", {"path": path, "metrics": {}, "activities": [], "missing": True}
            )
    ...
    return DashboardSection(
        "ready",
        {
            ...
            "metrics": parsed.frontmatter.get("metrics"),
            "activities": parsed.frontmatter.get("activities"),
...
def build_today_dashboard(
    *, vault_root: Path, runtime_dir: Path, inputs: TodayInputs
) -> TodayDashboard:
    ...
    try:
        ...
        revision_parts.extend(
            f"{item.source_path}:{item.task_id}:{item.status}" for item in actions
        )
    ...
    try:
        ...
        review = build_review_plan(
            cards=cards, as_of=inputs.day, available_minutes=inputs.study_minutes
        )
    ...
    try:
        ...
        active = tuple(
            {"path": path, "title": fm.get("title") or Path(path).stem}
            for path, fm in experiments_rows
            if str(fm.get("status", "active")).casefold() == "active"
        )
        ...
        inbox = DashboardSection(
            "ready" if inbox_items else "empty", {"count": len(inbox_items), "items": inbox_items}
        )
        proposal_rows = _frontmatter_rows(vault_root, ("proposals",))
        pending = tuple(
            {"path": path, "title": fm.get("title") or Path(path).stem, "status": fm.get("status")}
            for path, fm in proposal_rows
            if str(fm.get("status", "")).casefold() in {"draft", "pending", "approved"}
        )
        proposals = DashboardSection(
            "ready" if pending else "empty", {"count": len(pending), "items": pending}
        )
    ...
    attention_result = evaluate_attention(
        vault_root=vault_root,
        runtime_dir=runtime_dir,
        as_of=datetime.combine(inputs.day, datetime.now().time()).astimezone(),
    )
    attention = DashboardSection(
        "ready" if attention_result.items else "empty",
        {
            "count": len(attention_result.items),
            "items": tuple(asdict(item) for item in attention_result.items),
            "diagnostics": attention_result.diagnostics,
        },
    )
    ...
    diagnostics = DashboardSection(
        "ready" if diagnostics_items else "empty",
        {"count": len(diagnostics_items), "items": diagnostics_items},
    )
