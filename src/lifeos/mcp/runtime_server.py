...
LIFEOS_MCP_INSTRUCTIONS = (
    ...
    "write, delete, move, or shell surface. For a folder or several related sources, discover and "
    "read the selected canonical Markdown together, inspect applicable context and wiki knowledge, "
    "jointly reason about the durable delta, group that delta by target, then use "
    "ingestion_evolve_wiki_batch_proposal to create one reconciled draft. Do not loop the "
    "single-source proposal tool once per file. Zero durable changes remains a valid outcome. "
    "External research also remains agent-led: start with research_query_context when a single "
    "read-only research context is useful, or compose the lower-level context/search tools "
    "directly. When a material evidence gap requires an external source, use "
    "research_capture_evidence to preserve the selected source snapshot in raw/ with hash-bound "
    "acquisition lineage. If that research creates a reusable durable delta, use "
