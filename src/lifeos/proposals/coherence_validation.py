...
def preflight_proposal(
    ...
    *,
    ...
    Configured runtime targets are rejected from patch metadata before ordinary preflight can
    inspect ownership or target bytes. Identity discovery is likewise policy-scoped before
    Markdown is opened. An explicitly reviewed target may authorize protected-scope intent for
    that exact path only; unrelated protected or excluded notes cannot be read or influence the
    proposal result merely because they share the same frontmatter id. Config-aware callers pass
    ``runtime_dir`` explicitly so disposable custom-runtime Markdown stays outside canonical
    identity even when the config file itself lives outside the vault.
    """
    shell = _preflight_shell(proposal)
    try:
        runtime_prefix = runtime_exclusion_prefix(vault_root, runtime_dir=runtime_dir)
    except CoherenceError as error:
        return _invalidate(shell, code="runtime_scope_unresolvable", message=str(error))
    if runtime_prefix is not None:
        runtime_targets = sorted(
            {
                operation.target_path
                for operation in proposal.patch_document.operations
                if _target_inside_runtime(operation.target_path, runtime_prefix)
            }
        )
        if runtime_targets:
            return _invalidate(
...
def _preflight_shell(proposal: LoadedProposal) -> ProposalPreflightResult:
    """Create a content-free result shell for rejection before ordinary preflight."""
...
def _target_inside_runtime(target_path: str, runtime_prefix: str) -> bool:
    """Match a proposal target to the configured runtime using only lexical path metadata."""
