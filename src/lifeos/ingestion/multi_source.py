...
def _generated_create_candidate(
    *,
    ...
    parsed_type = None
    # Keep the existing page-kind convention without making it a batch taxonomy.
    from lifeos.wiki.layout import infer_wiki_page_kind

    parsed_type = infer_wiki_page_kind(target_path)
    frontmatter: dict[str, object] = {
        "title": mutation.content.title,
        **({"type": parsed_type} if parsed_type is not None else {}),
...
def build_multi_source_wiki_proposal(
    *,
    ...
    metadata = ProposalMetadata(
        ...
        title=(
            f"Reconcile {len(sources)} sources into {len(operations)} wiki target(s)"
        ),
    ...
    for index, item in enumerate(grounding, start=1):
        source_list = ", ".join(f"`{source['path']}`" for source in item["sources"])  # type: ignore[index]
