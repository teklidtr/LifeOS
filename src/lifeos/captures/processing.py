...
from .contracts import CaptureArtifact, CaptureError, CaptureState, CaptureType
...
class CaptureProcessingService:
    ...
    def apply_merge(self, preview: MergePreview, *, now: datetime | None = None) -> CaptureArtifact:
        ...
        merged = self.captures.create(
            ...
            source_entry_point="capture-merge",
        ...
        target.write_text(
            target.read_text().rstrip() + f"\n\n## Merged source annotations\n\n{source_notes}\n"
        )
        merged = self.captures.load(merged.path)
        metadata = replace(
            merged.metadata, merged_from=tuple(item.metadata.capture_id for item in artifacts)
        )
        ...
        for item in artifacts:
            ...
            if current.metadata.state != "archived":
                self.captures.transition(
                    ...
                    reason=f"merged into {merged.metadata.capture_id}",
                    now=now,
                )
        return merged
        ...
        *,
        ...
        for index, group in enumerate(groups, 1):
            created = self.captures.create(
                ...
                source_entry_point="capture-split",
            ...
            results.append(
                self.captures.save(
                    created,
                    replace(created.metadata, split_from=source.metadata.capture_id),
        ...
        self.captures.transition(
            ...
            "archived",
            expected_hash=current.content_hash,
            reason="split into new captures",
            now=now,
        )
