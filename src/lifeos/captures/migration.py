...
class CaptureMigrationPreview:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "legacy_formats_found": list(self.legacy_formats_found),
...
def preview_capture_migration(*, vault_root: Path, runtime_dir: Path) -> CaptureMigrationPreview:
    ...
    return CaptureMigrationPreview(
        (),
        (),
        "No repository-defined legacy rich-capture, meal-capture, workout-capture, or attachment-capture schema was found. Migration is a documented no-op.",
    )


def apply_capture_migration(
    *, vault_root: Path, runtime_dir: Path, expected_source_hashes: Mapping[str, str] | None = None
    ...
    if hashes:
        raise CaptureError(
            ...
            "No supported legacy rich-capture sources exist, so supplied source hashes cannot be applied.",
            {"paths": sorted(hashes)},
        )
    audit = runtime_dir / "captures" / "migration-audit.json"
    audit.parent.mkdir(parents=True, exist_ok=True)
    finding = (
        "No migration was required; no legacy format was invented and no source file was changed."
    )
    audit.write_text(
        json.dumps(
            {
    ...
    return CaptureMigrationResult(
        "not-required", (), (), (), (), str(audit.relative_to(runtime_dir)), finding
    )
