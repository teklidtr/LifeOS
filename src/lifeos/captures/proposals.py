...
class CaptureProposalService:
    ...
    def preview(self, request: CaptureProposalRequest, *, now: datetime | None = None) -> tuple[CaptureProposalPreview, PatchDocumentV2, bytes]:
        ...
        if any(item not in known for item in request.attachment_ids):
            raise CaptureError("unknown_attachment", "Proposal cites an attachment not linked to the capture.")
        moment = now or datetime.now(timezone.utc)
        if moment.tzinfo is None:
            raise CaptureError("invalid_timestamp", "Proposal timestamp must include a timezone.")
        moment = moment.astimezone(timezone.utc)
        suffix = hashlib.sha256("\0".join((capture.metadata.capture_id, capture.content_hash, request.action, request.target_path, body)).encode()).hexdigest()[:8]
        ...
        if request.create_target:
            ...
            operation = CreateFile("op-capture-create", request.target_path, "absent", body.rstrip() + "\n")
            ...
            try:
                ...
                raise CaptureError(exc.code, str(exc), {"target_path": request.target_path}) from exc
            ...
            operation = PatchHumanFile("op-capture-append", request.target_path, base_hash, "".join(diff[2:]))
        ...
        proposal_body = "\n".join(["# Capture evidence", "", f"- Capture: `[[{capture.path}]]`", f"- Capture hash: `{capture.content_hash}`", f"- Attachments: {', '.join(request.attachment_ids) or 'none'}", "", "## Proposed action", "", f"- Target: `{request.target_path}`", f"- Operation: `{operation_name}`", f"- Included: {', '.join(request.included_actions) or 'none listed'}", f"- Excluded: {', '.join(request.excluded_actions) or 'none listed'}", "", "```markdown", body, "```", "", "No external canonical artifact changes until this proposal is reviewed, approved, and applied."])
        preview = CaptureProposalPreview(proposal_id, request.target_path, operation_name, base_hash, capture.metadata.capture_id, capture.content_hash, request.attachment_ids, request.included_actions, request.excluded_actions)
        return preview, patch, serialize_proposal_markdown(metadata, proposal_body)

    def publish(self, request: CaptureProposalRequest, *, now: datetime | None = None) -> dict[str, object]:
        ...
        try:
            ...
            raise CaptureError("proposal_exists", "Equivalent capture proposal already exists.") from exc
