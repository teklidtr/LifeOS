...
class ComparisonDimension:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "values": [{"option_id": key, "value": value} for key, value in self.values],
        }
...
def explain_plan_option(
    *,
    ...
    for ref in option.source_refs:
        option_provenance.append(
            _entry(
                ...
                "canonical-note",
                ref,
                "This source informed the visible option.",
                sources,
            )
        )
    for assumption in option.assumptions:
        ...
        option_provenance.append(
            _entry(
                f"option.assumptions.{assumption.assumption_id}",
                assumption.source_kind,
    ...
    if not option_provenance:
        option_provenance.append(
            ProvenanceEntry(
                ...
                "deterministic-fact",
                None,
                "No supporting source was attached to this sparse option.",
                "missing",
            )
        )
    ...
    items: list[ItemExplanation] = [
        ItemExplanation(
            option.option_id,
            "option",
            f"{option.title}: {option.strategy}",
            tuple(sorted(option_provenance, key=_provenance_key)),
        )
    ]
    for milestone in sorted(option.milestones, key=lambda item: item.milestone_id):
        provenance = tuple(
            _entry(
                f"milestones.{milestone.milestone_id}",
                "deterministic-fact",
                ref,
                "The milestone is part of the selected rolling-wave option.",
                sources,
            )
            for ref in option.source_refs
        ) or (
            ProvenanceEntry(
                ...
                "deterministic-fact",
                None,
                "Generated from the visible option structure.",
                "supported",
            ),
        )
        items.append(
            ItemExplanation(milestone.milestone_id, "milestone", milestone.outcome, provenance)
        )
    for generated in sorted(decomposition.actions, key=lambda item: item.action.task_id):
        ...
        provenance = tuple(
            _entry(
                f"actions.{action.task_id}",
                "deterministic-fact",
                ref,
                action.rationale or "Generated for the current rolling-wave milestone.",
                sources,
            )
            for ref in refs
        ) or (
            ProvenanceEntry(
                ...
                "deterministic-fact",
                None,
                action.rationale or "Generated from the selected milestone.",
                "supported",
            ),
        )
        items.append(
            ItemExplanation(
                action.task_id,
                "action",
                f"{action.title}. Evidence: {generated.verification}",
                provenance,
            )
        )
    for finding in capacity.findings:
        provenance = tuple(
            ProvenanceEntry(
                f"capacity.{finding.code}",
                "deterministic-fact",
                ref,
                "This visible input contributed to the capacity finding.",
                "supported",
            )
            ...
            ProvenanceEntry(
                ...
                "deterministic-fact",
                None,
                "The finding is based on missing or aggregate visible inputs.",
                "missing" if finding.missingness else "supported",
            ),
        )
    ...
    omissions = tuple(
        OmissionSummary(item.path, item.reason, _omission_effect(item.reason))
        for item in context.omissions
    )
    views = (f"Baseline fit: {capacity.baseline.fit}.",) + (
        (f"Adaptive fit: {capacity.adaptive.fit}.",) if capacity.adaptive else ()
    )
    ...
    *,
    ...
    dimensions: tuple[ComparisonDimension, ...] = (
        _dimension(
            ...
            ordered,
            lambda o: (
                f"{len(o.milestones)} milestones; {len(decompositions[o.option_id].actions)} near-term actions"
            ),
        ),
        _dimension(
            "pace",
            ordered,
            lambda o: f"{decompositions[o.option_id].current_window_days}-day current window",
        ),
        _dimension(
            "uncertainty",
            ordered,
            lambda o: f"{o.confidence_label} confidence; {len(o.assumptions)} assumptions",
        ),
        ...
        _dimension(
            ...
            ordered,
            lambda o: (
                decompositions[o.option_id].actions[0].action.title
                if decompositions[o.option_id].actions
                else "No feasible first step"
            ),
        ),
        _dimension(
            "unresolved-questions", ordered, lambda o: "; ".join(o.unresolved_questions) or "None"
        ),
    )
    return PlanOptionComparison(
        1,
        tuple(item.option_id for item in ordered),
        dimensions,
        "No winner is assigned. Compare the visible dimensions against your own criteria.",
    )
...
def recompute_capacity_counterfactual(
    *,
    ...
    after = check_portfolio_capacity(
        ...
        recurring_workloads=before.recurring_workloads
        if recurring_workloads is None
        else recurring_workloads,
    ...
    return CounterfactualResult(
        ...
        before.fit,
        after.fit,
        tuple(sorted(before_codes ^ after_codes)),
        after,
    )
...
def _source_states(
    context: PlanningContextPack,
) -> dict[str, Literal["supported", "stale", "excluded", "deleted"]]:
    ...
    for omission in context.omissions:
        result[omission.path] = (
            "excluded"
            if "excluded" in omission.reason or "denied" in omission.reason
            else "deleted"
        )
    return result
...
def _contradictions(
    option: PlanOption, capacity: PortfolioCapacityReport
) -> tuple[ContradictionSummary, ...]:
    ...
    if capacity.fit == "overload" and capacity_assumptions:
        findings.append(
            ContradictionSummary(
                "capacity-assumption-conflict",
                "A visible capacity assumption conflicts with the current overload finding.",
                tuple(a.assumption_id for a in capacity_assumptions),
            )
        )
    if option.review_date and any(
        m.target_date and m.target_date > option.review_date for m in option.milestones
    ):
        findings.append(
            ContradictionSummary(
                "review-before-milestone",
                "The plan review occurs before a stated milestone target.",
                tuple(
                    m.milestone_id
                    for m in option.milestones
                    if m.target_date and m.target_date > option.review_date
                ),
            )
        )
...
def _dimension(
    ...
    options: Sequence[PlanOption],
    render: Callable[[PlanOption], str],
) -> ComparisonDimension:
    return ComparisonDimension(
        name, tuple((option.option_id, render(option)) for option in options)
    )
