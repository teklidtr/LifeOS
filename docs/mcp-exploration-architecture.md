...
`vault_read_markdown`, `vault_context`, and `runtime_activity` entries with policy-aware adapters
over the same underlying state, then adds the four exploration primitives.
...
facade results back to structured MCP output. User-facing activity reads reapply the current
external retrieval policy to path metadata so a prior one-request protected read does not become
a later discovery side channel.
...
The complete user-facing read surface enforces the canonical retrieval policy as an external
disclosure boundary. This includes focused single-note reads, focused/lexical context assembly,
activity path metadata, and the broad exploration tools. Excluded prefixes remain unavailable.
Protected prefixes are default-deny. MCP tools that support protected reads require both an
explicit `allow_protected=true` request and a matching `external_allowed_prefixes` policy entry.

Policy filtering occurs before path descent/content decoding and before lexical ranking/result
caps. Disallowed candidates therefore cannot crowd allowed results out of a bounded search or
context pack, and denied Markdown/YAML cannot surface through parser or decode diagnostics.
Retrieval policy itself is loaded through descriptor-based symlink-safe vault I/O and fails
closed when unsafe or malformed.
...
- `vault_search`: at most 50 returned hits, with titles capped at 512 characters, descriptions at
  1,024 characters, bounded excerpts, and bounded parser diagnostics;
- `vault_read_many`: at most eight paths and at most 100,000 returned Markdown-body characters,
  with separately bounded title metadata;
- `vault_links`: at most 100 references per request plus bounded omission diagnostics for allowed
  backlink candidates that cannot be read or parsed.

`vault_list` uses deterministic path ordering. When a page is truncated it returns `next_after`;
passing that value back as `after` continues after the last returned path. `vault_links` likewise
uses deterministic ordering and returns `next_offset` for a truncated page. These continuation
contracts let an MCP-only client complete exact discovery without already knowing omitted names.

## Canonical link resolution

`vault_links` preserves link syntax before resolution. A normal Markdown link resolves relative
to its source note unless it is explicitly vault-rooted. A path-qualified Obsidian wikilink is
interpreted as a canonical vault path and is not silently reinterpreted as source-relative. A
basename-only wikilink such as `[[topic]]` may fall back to basename resolution only when exactly
one allowed canonical Markdown path has that basename. Ambiguous or unresolved targets are not
guessed. The same canonicalized target is used for outgoing links and backlink comparisons.

Requested-source read/parse failures fail deterministically. During backlink scans, failures in
other allowed candidates are represented as bounded structured diagnostics instead of being
silently indistinguishable from a complete backlink set.
...
- focused and broad MCP reads use external disclosure semantics and cannot bypass protected-scope
  policy with request intent alone;
- protected/excluded Markdown and YAML are filtered before decode or structural diagnostics;
- retrieval policy loading rejects symlink escapes and malformed canonical policy state;
...
- search and multi-read metadata remain bounded even with oversized frontmatter;
- truncated path and link discovery can continue deterministically;
- backlink scans signal malformed allowed candidates through bounded diagnostics;
- runtime activity re-filters path metadata so prior protected reads cannot leak path names;
- invalid bounded exploration arguments are reported as tool argument errors and MCP input types
  are strict rather than coercive;
- unique basename wikilinks resolve to canonical paths while ambiguous basenames are not guessed,
  and Markdown-relative versus wikilink-root semantics remain distinct;
