...
- registry migrations and provenance indexing tests;
...
| Canonical domain inventory | Architecture's example vault list omitted implemented canonical areas used by proposals, knowledge conversations, rich captures, and attachment evidence. | Expanded the architecture inventory and distinguished first-party bootstrap roots from feature-created canonical areas. |
| First-party bootstrap | Setup already accurately documents `lifeos init`, but the repository README still reads primarily as an implementation bootstrap/scaffold and did not offer the current first-party vault quick start. The durable non-destructive bootstrap policy also existed mainly in LIFEOS-1634 plus code. | Reframed README around the current product/application-vs-vault model, added the supported quick start, and promoted the bootstrap contract into `docs/design-decisions.md`. |
