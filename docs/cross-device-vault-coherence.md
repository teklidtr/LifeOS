...
| `.lifeos/` registry, retrieval indexes, embeddings, journals, runtime activity | Disposable | Node-local; exclude from sync when runtime storage lives under the vault |
...
because `.lifeos/` is explicitly disposable and ignored by Git. A deployment may instead set
`runtime_dir` outside the vault. `lifeos doctor` reports which topology is active.
...
A sync provider may require additional provider-specific exclusions. Those remain deployment
configuration rather than LifeOS business rules.
...
When a scan sees exactly one existing registry record with a stable ID at a new path, the same
registry row is reconciled to the new path. Pure moves are reported separately from new/deleted
files. Move-plus-edit records both relocation and modification. Rebuilding the database from
Markdown must reproduce the same identity facts.
...
| Same ID resolves to multiple paths | Ambiguous identity; fail closed |
...
- **Disposable-state corruption:** delete/rebuild node-local state from canonical Markdown and Git
  history rather than treating `.lifeos/` as authoritative.
