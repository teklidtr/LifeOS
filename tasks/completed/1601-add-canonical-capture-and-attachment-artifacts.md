---
id: LIFEOS-1601
title: Add canonical capture and attachment artifacts
status: completed
phase: 16
depends_on:
  - LIFEOS-1600
...
Versioned capture and attachment contracts, Markdown persistence, stable identity, human-owned regions, and optimistic writes.
...
- `PYTHONPATH=src python3 -m pytest -q tests/captures/test_artifact.py`: 6 passed.
- Ruff was unavailable in the sandbox; `uv run ruff` could not obtain its locked Python runtime because outbound DNS was unavailable. A later final gate retries the check if tooling becomes available.
