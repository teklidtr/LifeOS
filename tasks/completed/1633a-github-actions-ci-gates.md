...
- Implementing the LIFEOS-1634 vault bootstrap.
...
- A failed job can be diagnosed from GitHub Actions logs without requiring a local Docker
  reproduction first.
...
separately by LIFEOS-1616.

# Relevant decisions

- GitHub Actions is the canonical clean-room CI environment; local Docker remains useful
  but is no longer required for ordinary pull-request diagnosis.
...
- Python 3.11 remains supported, so MCP/Pydantic schema code and clean-room tests run on
  Python 3.11.
- The project pins MCP to the 1.x SDK (`mcp>=1.28,<2`).

# Completion notes

- Implemented and merged through pull request #1.
- Final pre-merge CI run passed both `test` and `docker-setup-e2e`.
- The full pytest suite passed with 1504 tests on the final PR head.
- Docker clean-room validation passed the fresh-vault setup contract and real MCP suite.
- Pull request #1 was merged into `master` at merge commit
  `e8a43d3bb15e34484a1c303126bec92fa1148273`.
- LIFEOS-1634 is now eligible to be selected from `tasks/ready/`.
