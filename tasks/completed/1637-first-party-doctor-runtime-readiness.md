...
Architecture documentation did not require a separate change: the implementation composes
existing bootstrap and `collect_status()` contracts without introducing a new canonical data,
mutation, ownership, registry, or agent boundary. README and Setup & Installation were updated
for the new user-visible CLI behavior.
...
GitHub Actions CI on PR #8 passed all required gates before completion:

- documentation impact gate: passed;
- Ruff repository gate: passed;
- mypy: no issues in 184 source files;
- Python compileall: passed;
- manual links: 15 chapters validated;
- full pytest suite: 1531 passed;
- Docker fresh-vault setup gate: 3 passed;
- Docker real MCP gate: 51 passed.

The completed implementation provides a read-only first-party doctor command, shares vault
bootstrap recognition with `lifeos init`, composes existing status semantics, reports optional
MCP readiness without making MCP mandatory, and preserves explicit client-specific MCP
configuration and actor identity.
...
- LIFEOS-1636 reconciled current setup, registry, MCP, bootstrap, and operational docs with
  shipped behavior; 1637 extends those current contracts rather than reviving stale setup
  paths.
