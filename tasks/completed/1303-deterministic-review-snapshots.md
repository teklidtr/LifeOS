...
Turn review inputs into bounded, provenance-rich snapshots that can be refreshed without overwriting human review work.

# Scope

- Refactor existing review section gathering behind typed snapshot contracts.
- Capture source path, source hash, evidence time, item fingerprint, state, and diagnostics.
- Make partial subsystem failure section-local.
- Store snapshot identity and refresh history in the artifact.
