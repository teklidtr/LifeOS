...
- Define the folder-ingestion orchestration contract for an external MCP agent:
  ...
  3. read and reason over the selected sources jointly, with applicable vault context;
...
- Add a multi-source proposal-producing facade/MCP contract rather than looping over the existing
  ...
  but the contract must accept a bounded ordered set of source paths plus target-centric agent
  mutations.
...
- Verify all selected sources immediately before proposal publication. Any missing,
  unregistered, modified, policy-denied, duplicate/ambiguous identity, unsafe, or unreadable
  source blocks the whole draft before proposal persistence.
...
- Update MCP instructions so a folder request is described as joint source exploration and one
  target-reconciled draft, not independent ingestion of every file.
...
- An MCP-only external agent can take a folder containing multiple eligible Markdown sources,
  inspect them with existing exploration/context/search tools, and create **one** draft knowledge
  proposal grounded in multiple verified source snapshots.
...
- Proposal review artifacts expose a deterministic target-to-source grounding map plus mutation
  rationale and bind it to the ordinary review digest without exposing hidden reasoning.
...
- MCP guidance explicitly describes folder ingestion as `discover/read sources -> inspect
  context/wiki -> jointly reason -> group by target -> one draft`, and states that zero durable
  changes remains a valid result.
