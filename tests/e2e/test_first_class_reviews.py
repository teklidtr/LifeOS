...
def test_daily_review_survives_human_edits_runtime_deletion_and_rebuild(tmp_path: Path) -> None:
    ...
    path.write_text(
        path.read_text(encoding="utf-8").replace(
            "### Orientation", "### Orientation\n\nProtect the first hour for reading."
        ),
        encoding="utf-8",
    )

    loaded = app.dispatch(
        "review.artifact.load", {"path": opened["artifact"]["path"], "now": NOW.isoformat()}
    )
    ...
    artifact = app.dispatch(
        "review.artifact.phase",
        {
            ...
            "phase_id": "morning",
            "action": "complete",
            "required_sections": ["attention", "plans"],
            "expected_hash": artifact["content_hash"],
            "now": NOW.isoformat(),
            "idempotency_key": "e2e-morning-complete",
        },
    )
    ...
    artifact = app.dispatch(
        "review.artifact.phase",
        {
            ...
            "phase_id": "evening",
            "action": "complete",
            "required_sections": ["daily-evidence", "attention"],
            "expected_hash": artifact["content_hash"],
            "now": EVENING.isoformat(),
            "idempotency_key": "e2e-evening-complete",
        },
    )
    artifact = app.dispatch(
    ...
    resumed = app.dispatch(
        "review.artifact.load", {"review_id": "daily-2026-07-16", "now": EVENING.isoformat()}
    )
...
def test_review_proposal_is_draft_and_external_target_stays_unchanged(tmp_path: Path) -> None:
    ...
    proposal = app.dispatch(
        "review.proposal.create",
        {
            ...
            "value": "archived",
            "rationale": "This capture was reviewed.",
            "now": NOW.isoformat(),
        },
    )
    assert target.read_text(encoding="utf-8") == original
    loaded = load_proposal_directory(
        tmp_path / proposal["proposal_path"], proposals_root=tmp_path / "proposals"
    )
    ...
    decided = app.dispatch(
        "review.artifact.decide",
        {
            ...
            "expected_hash": artifact.content_hash,
            "idempotency_key": "e2e-attach-proposal",
            "now": NOW.isoformat(),
        },
    )
...
def test_sparse_degraded_concurrent_and_migrated_paths_fail_safe(tmp_path: Path) -> None:
    sparse = BridgeApplication(
        vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos", actor_id="tester"
    )
    opened = open_review(sparse)
    assert opened["artifact"]["path"] == "reviews/daily/2026-07-16.md"
    assert all(
        section["state"] in {"empty", "ready", "unavailable"}
        for section in opened["snapshot"]["sections"]
    )
    ...
    expected = {
        source["path"]: source["content_hash"] for source in preview["candidates"][0]["sources"]
    }
    migrated = sparse.dispatch(
        "review.artifact.migration.apply",
        {
            "now": NOW.isoformat(),
            "idempotency_key": "e2e-migrate",
