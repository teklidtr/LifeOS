...
from lifeos.captures.integrations import CaptureExperimentMapping, CaptureLinkService, capture_as_experiment_observation
from lifeos.captures.proposals import CaptureProposalRequest, CaptureProposalService
from lifeos.captures.retrieval_integration import build_capture_representation, chunk_capture_representation, conversation_evidence
...
def test_linking_is_explicit_and_deduplicated(tmp_path: Path) -> None:
    ...
    linked = service.link(capture.path, ArtifactLink("diary/2026-07-16.md", "supports", "diary"), expected_hash=capture.content_hash, now=NOW)
    same = service.link(linked.path, linked.metadata.links[0], expected_hash=linked.content_hash, now=NOW)
...
def test_retrieval_indexes_only_approved_text_and_marks_stale_extraction(tmp_path: Path) -> None:
    ...
    capture = captures.save(capture, replace(capture.metadata, derived_values=(suggested, confirmed)), expected_hash=capture.content_hash)
    ...
    stale = build_capture_representation(capture, extractions=(ExtractionResult("unknown", "sha256:" + "0" * 64, "ocr", "1", "stale"),))
...
def test_semantic_exclusion_and_conversation_provenance(tmp_path: Path) -> None:
    captures = CaptureArtifactService(vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos")
    capture = captures.create(title="Audio", capture_type="attachment", description="voice note", now=NOW)
    evidence = conversation_evidence(build_capture_representation(capture))
    assert evidence.representation_kinds == ("user-description",)
    excluded = captures.save(capture, replace(capture.metadata, exclude_from_semantic=True), expected_hash=capture.content_hash)
...
def test_daily_and_weekly_reviews_are_optional_and_non_moralizing(tmp_path: Path) -> None:
    ...
    daily = daily_capture_section(vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos", day=date(2026, 7, 16), generated_at=NOW)
    weekly = weekly_capture_section(vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos", range_start=date(2026, 7, 13), range_end=date(2026, 7, 19), generated_at=NOW)
    assert daily.optional and len(daily.items) == 2
    assert weekly.optional and "good" not in weekly.items[0].detail.casefold() and "bad" not in weekly.items[0].detail.casefold()
...
def experiment(tmp_path: Path):
    ...
    return ExperimentArtifactService(vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos").create(title="Walking", description="", category="activity", protocol=protocol, now=NOW)
...
def test_experiment_mapping_requires_visible_confirmation(tmp_path: Path) -> None:
    ...
    capture = captures.save(capture, replace(capture.metadata, derived_values=(DerivedValue("mood", 7, None, "model-estimate", "medium"),)), expected_hash=capture.content_hash)
    ...
    confirmed = captures.save(capture, replace(capture.metadata, derived_values=(replace(capture.metadata.derived_values[0], status="confirmed"),)), expected_hash=capture.content_hash)
...
def test_capture_proposal_is_source_hash_and_target_hash_guarded(tmp_path: Path) -> None:
    ...
    target = tmp_path / "notes" / "book.md"; target.parent.mkdir(); target.write_text("# Book\n")
    service = CaptureProposalService(vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos", actor_id="tester")
    request = CaptureProposalRequest(capture.path, "append-note", "notes/book.md", "## Finding\n\nCheck this claim.", included_actions=("append reviewed text",), excluded_actions=("change capture",))
    ...
    assert patch.operations[0].base_hash != "sha256:" + __import__("hashlib").sha256(target.read_bytes()).hexdigest()
