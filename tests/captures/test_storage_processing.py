...
def services(
    tmp_path: Path,
) -> tuple[CaptureArtifactService, AttachmentStore, CaptureProcessingService]:
    runtime = tmp_path / ".lifeos"
    return (
        CaptureArtifactService(vault_root=tmp_path, runtime_dir=runtime),
        AttachmentStore(vault_root=tmp_path, runtime_dir=runtime),
        CaptureProcessingService(vault_root=tmp_path, runtime_dir=runtime),
    )


def test_exact_duplicate_reuses_original_and_same_name_different_content_does_not(
    tmp_path: Path,
) -> None:
    _, store, _ = services(tmp_path)
    first = tmp_path / "source-a" / "receipt.txt"
    first.parent.mkdir()
    first.write_text("one")
    second = tmp_path / "source-b" / "receipt.txt"
    second.parent.mkdir()
...
def test_attach_remove_and_safe_delete_reference_checks(tmp_path: Path) -> None:
    ...
    linked = store.attach_to_capture(
        capture.path, imported.reference, expected_hash=capture.content_hash, now=NOW
    )
    ...
    unlinked = store.remove_from_capture(
        capture.path, imported.reference.attachment_id, expected_hash=linked.content_hash, now=NOW
    )
...
def test_local_text_and_image_metadata_extraction(tmp_path: Path) -> None:
    ...
    text_manifest = store.manifests.load(
        store.import_file(text, capture_source="drop", now=NOW).manifest_path
    )
    image_manifest = store.manifests.load(
        store.import_file(png, capture_source="drop", now=NOW).manifest_path
    )
...
def test_pdf_extraction_records_page_provenance_without_real_dependency(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _, store, _ = services(tmp_path)
    pdf = tmp_path / "notes.pdf"
    pdf.write_bytes(b"%PDF-1.4\nfixture")
    manifest = store.manifests.load(
        store.import_file(pdf, capture_source="drop", now=NOW).manifest_path
    )
    ...
    module.PdfReader = FakePdfReader  # type: ignore[attr-defined]
    module.__version__ = "fixture-1"
    monkeypatch.setitem(sys.modules, "pypdf", module)

    result = LocalExtractionService(vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos").extract(
        manifest.metadata
    )
    assert result.status == "completed"
    assert result.method == "pypdf"
    assert result.method_version == "fixture-1"
    assert result.source_locator == "page"
    assert result.text == "[Page 1]\nfirst\n\n[Page 2]\nsecond"


def test_audio_metadata_preserves_original_without_inventing_transcript(tmp_path: Path) -> None:
    _, store, _ = services(tmp_path)
    ...
    manifest = store.manifests.load(
        store.import_file(audio, capture_source="voice-note", now=NOW).manifest_path
    )
    result = LocalExtractionService(vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos").extract(
        manifest.metadata
    )
    ...
    assert (tmp_path / manifest.metadata.canonical_path).read_bytes() == audio.read_bytes()


def test_unsupported_and_oversized_files_remain_preserved(tmp_path: Path) -> None:
    _, store, _ = services(tmp_path)
    arbitrary = tmp_path / "data.unknown"
    arbitrary.write_bytes(b"abc")
    big = tmp_path / "big.txt"
    big.write_text("x" * 20)
    a = store.manifests.load(
        store.import_file(arbitrary, capture_source="drop", now=NOW).manifest_path
    )
    b = store.manifests.load(store.import_file(big, capture_source="drop", now=NOW).manifest_path)
    extractor = LocalExtractionService(
        vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos", max_text_bytes=10
    )
...
def test_interrupted_processing_resumes_without_losing_capture(tmp_path: Path) -> None:
    ...
    two.write_text("two")
    refs = (
        store.import_file(one, capture_source="drop", now=NOW).reference,
        store.import_file(two, capture_source="drop", now=NOW).reference,
    )
...
def test_merge_and_split_preserve_references_and_archive_sources(tmp_path: Path) -> None:
    ...
    assert {item.attachment_id for item in merged.metadata.attachments} == {
        a_ref.attachment_id,
        b_ref.attachment_id,
    }
    ...
    parts = processing.split(
        merged.path,
        ((a_ref.attachment_id,), (b_ref.attachment_id,)),
