...
def test_evidence_only_no_provider_and_no_answer_states(tmp_path: Path) -> None:
    ...
    evidence_only = service.ask(path, query="ATP", expected_hash=digest, evidence_only=True, now=NOW)
    ...
    no_results = service.ask(path, query="xyzzynotfound", expected_hash=unavailable.content_hash, now=NOW)
...
def test_valid_claim_citations_are_saved_and_nonexistent_citations_rejected(tmp_path: Path) -> None:
    _, service, path, digest = setup(tmp_path)
    preview = service.retriever.search(__import__('lifeos.retrieval', fromlist=['RetrievalRequest']).RetrievalRequest("ATP"))
    evidence_id = preview.results[0].evidence_id
    good = DeterministicAnswerProvider(
        GeneratedAnswer((GeneratedParagraph("ATP is produced.", (evidence_id,), "direct"),), "Validated.")
    )
    ...
    bad = DeterministicAnswerProvider(
        GeneratedAnswer((GeneratedParagraph("Invented.", ("chunk:missing",), "direct"),), "Invalid.")
    )
    rejected = service.ask(path, query="ATP", expected_hash=saved.content_hash, answer_provider=bad, now=NOW)
...
def test_timeout_and_stale_source_detection(tmp_path: Path) -> None:
    ...
    timeout = service.ask(
        path, query="ATP", expected_hash=digest, answer_provider=FailingAnswerProvider("timeout"), now=NOW
    )
    assert timeout.turns[-1].state == "timeout"
    prior = service.ask(path, query="ATP", expected_hash=timeout.content_hash, evidence_only=True, now=NOW)
