...
def test_review_artifact_bridge_open_resume_progress_and_history(tmp_path: Path) -> None:
    ...
    updated = bridge.call(
        ...
        idempotency_key="bridge-review-section",
        now="2026-07-16T08:05:00+03:00",
    )
    bridge.call(
        ...
        idempotency_key="bridge-review-answer",
        now="2026-07-16T08:06:00+03:00",
    )
    loaded = bridge.call(
        "review.artifact.load",
        review_id=artifact["metadata"]["review_id"],
        now="2026-07-16T08:07:00+03:00",
    )
...
def test_review_artifact_bridge_stale_write_is_actionable(tmp_path: Path) -> None:
    ...
    opened = bridge.call(
        ...
        day="2026-07-16",
        timezone="Europe/Istanbul",
        now="2026-07-16T18:00:00+03:00",
        refresh=True,
        idempotency_key="weekly-open",
    )
    with pytest.raises(ProtocolError) as caught:
        bridge.call(
            "review.artifact.refresh",
            review_id=opened["artifact"]["metadata"]["review_id"],
            expected_hash="0" * 64,
            now="2026-07-16T18:05:00+03:00",
            idempotency_key="weekly-refresh",
        )
...
def test_review_migration_and_rebuild_are_explicit_bridge_actions(tmp_path: Path) -> None:
    app, bridge = client(tmp_path)
    reviews = app.daily.vault_root / "reviews"
    reviews.mkdir()
    ...
    applied = bridge.call(
        ...
        now="2026-07-16T12:00:00+03:00",
        idempotency_key="bridge-migration",
