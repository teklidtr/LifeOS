...
def protocol() -> dict[str, object]:
    return {
        ...
        "hypothesis": "Focus is higher after walking.",
        ...
        "adherence_expectation": "Both days",
        "confounders": ["sleep"],
        "risks": [],
...
def test_experiment_bridge_vertical_slice_and_capabilities(tmp_path: Path) -> None:
    ...
    created = bridge.call(
        "experiment.create", title="Walk", category="study", protocol=protocol(), now=NOW
    )
    ...
    baseline = bridge.call(
        "experiment.transition",
        path=drafted["path"],
        target="baseline",
        expected_hash=drafted["content_hash"],
        now=NOW,
    )
    ...
    due = bridge.call(
        "experiment.schedule.due", path=observed["path"], now="2026-07-16T20:00:00+00:00"
    )
    assert isinstance(due, list)
    preview = bridge.call(
        "experiment.proposal.preview",
        experiment_path=observed["path"],
        action="create-knowledge-note",
        target_path="notes/walk.md",
        content="# Walk finding",
        create_target=True,
    )
...
def test_bridge_rejects_extra_fields_stale_writes_and_unsafe_activation(tmp_path: Path) -> None:
    ...
    unsafe = protocol()
    unsafe["intervention"] = "Stop prescription medication and change dose"
...
def test_experiment_migration_privacy_and_recovery_bridge_methods(tmp_path: Path) -> None:
    ...
    denied = bridge.call(
        "experiment.privacy.preview",
        experiment_path=created["path"],
        selected_paths=["diary/day.md"],
    ...
    assert (
        "PrivateName"
        not in next(item for item in allowed["items"] if item["path"] == "diary/day.md")["excerpt"]
    )
...
def test_experiment_privacy_bridge_rejects_unbounded_or_malformed_inputs(tmp_path: Path) -> None:
    ...
    with pytest.raises(ProtocolError) as malformed:
        bridge.call(
            "experiment.privacy.preview",
            experiment_path=created["path"],
            selected_paths="diary/x.md",
        )
    assert malformed.value.code == "invalid_params"
    with pytest.raises(ProtocolError) as budget:
        bridge.call(
            "experiment.privacy.preview", experiment_path=created["path"], max_total_bytes=0
        )
