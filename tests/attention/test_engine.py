...
""",
    ...
    app.record_task_outcome(
        TaskOutcomeRequest(
            "event-1", "plans/p.md", "t", "done", now.date(), content_hash(plan.read_text())
        )
    )
    assert not any(
        item.kind == "unaccounted_task"
        for item in evaluate_attention(vault_root=vault, runtime_dir=runtime, as_of=now).items
    )
...
def test_independent_items_survive_bad_planning(tmp_path: Path) -> None:
    ...
    result = evaluate_attention(
        vault_root=vault, runtime_dir=runtime, as_of=datetime(2026, 7, 16, 22, tzinfo=timezone.utc)
    )
