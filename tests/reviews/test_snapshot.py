...
def test_snapshot_has_source_hashes_fingerprints_and_section_local_diagnostics(
    tmp_path: Path,
) -> None:
...
def test_refresh_updates_only_managed_blocks_and_keeps_bounded_history(tmp_path: Path) -> None:
    ...
    refreshed, snapshot = refresh_review_snapshot(
        service=service,
        artifact=artifact,
        runtime_dir=runtime,
        generated_at=NOW,
    ...
    same, _ = refresh_review_snapshot(
        service=service,
        artifact=refreshed,
        runtime_dir=runtime,
        generated_at=NOW,
        idempotency_key="refresh-same",
    )
