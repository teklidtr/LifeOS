...
def test_preview_and_apply_merge_daily_reflections_without_deleting_sources(tmp_path: Path) -> None:
    ...
    morning = reviews / "morning-2026-07-16.md"
    evening = reviews / "evening-2026-07-16.md"
    morning.write_text(
        legacy("morning", "2026-07-16", "Protect the quiet morning."), encoding="utf-8"
    )
    evening.write_text(
        legacy("evening", "2026-07-16", "Reading happened; errands moved."), encoding="utf-8"
    )
    ...
    result = apply_review_migration(
        vault_root=vault, runtime_dir=runtime, actor_id="me", now=NOW, idempotency_key="migration"
    )
    assert result.migrated == ("daily-2026-07-16",)
    assert morning.exists() and evening.exists()
    artifact = ReviewArtifactService(vault_root=vault, runtime_dir=runtime).load_id(
        "daily-2026-07-16"
    )
    assert "Protect the quiet morning." in artifact.body
    assert "Reading happened; errands moved." in artifact.body
    assert set(artifact.metadata.migrated_from) == {
        "reviews/morning-2026-07-16.md",
        "reviews/evening-2026-07-16.md",
    }
    assert "Managed old fact" not in artifact.body

    repeated = apply_review_migration(
        vault_root=vault,
        runtime_dir=runtime,
        actor_id="me",
        now=NOW,
        idempotency_key="migration-again",
    )
...
def test_weekly_legacy_note_uses_iso_week_identity(tmp_path: Path) -> None:
    ...
    source.write_text(
        legacy("weekly", "2026-07-13", "The week bent toward study."), encoding="utf-8"
    )
    result = apply_review_migration(
        vault_root=vault,
        runtime_dir=runtime,
        actor_id="me",
        now=NOW,
        idempotency_key="weekly-migration",
    )
...
def test_existing_non_pristine_target_and_malformed_legacy_fail_closed(tmp_path: Path) -> None:
    ...
    artifact = service.open_or_create(
        kind="daily", day=NOW.date(), timezone="Europe/Istanbul", now=NOW, idempotency_key="open"
    )
    ...
    (reviews / "morning-2026-07-16.md").write_text(
        legacy("morning", "2026-07-16", "Legacy text"), encoding="utf-8"
    )
    ...
    result = apply_review_migration(
        vault_root=vault, runtime_dir=runtime, actor_id="me", now=NOW, idempotency_key="conflict"
    )
...
def test_rebuild_restores_disposable_indexes_from_markdown(tmp_path: Path) -> None:
    ...
    service.open_or_create(
        kind="daily", day=NOW.date(), timezone="Europe/Istanbul", now=NOW, idempotency_key="daily"
    )
    service.open_or_create(
        kind="weekly", day=NOW.date(), timezone="Europe/Istanbul", now=NOW, idempotency_key="weekly"
    )
    if runtime.exists():
        import shutil
...
def test_migration_detects_stale_preview_and_resumes_pristine_target(tmp_path: Path) -> None:
    ...
    service.open_or_create(
        ...
        now=NOW,
        idempotency_key="interrupted-open",
    )
    ...
    resumed = apply_review_migration(
        vault_root=vault, runtime_dir=runtime, actor_id="me", now=NOW, idempotency_key="resume"
    )
