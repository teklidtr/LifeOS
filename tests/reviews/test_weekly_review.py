...
def setup(tmp_path: Path):
    ...
    write(
        vault / "experiments" / "sleep.md",
        "---\ntype: experiment\ntitle: Sleep experiment\nstatus: active\nrequired_metrics: [sleep_hours]\n---\n",
    )
    return (
        vault,
        runtime,
        ReviewArtifactService(vault_root=vault, runtime_dir=runtime, actor_id="me"),
    )
...
def test_weekly_review_uses_iso_identity_and_bounded_sections(tmp_path: Path) -> None:
    ...
    state = open_weekly_review(
        ...
        day=date(2026, 1, 1),
        timezone="UTC",
        now=datetime(2026, 1, 4, 18, tzinfo=timezone.utc),
...
def test_weekly_evidence_summarizes_outcomes_and_active_experiments(tmp_path: Path) -> None:
    ...
    DailyInteractionService(vault_root=vault, runtime_dir=runtime).record_task_outcome(
        TaskOutcomeRequest(
            "outcome",
            "plans/plan.md",
            "t1",
    ...
    execution = next(
        section for section in state.snapshot.sections if section.section_id == "weekly-evidence"
    )
    assert "done: 1" in execution.items[0].detail
    experiments = next(
        section for section in state.snapshot.sections if section.section_id == "experiments"
    )
...
def test_weekly_completion_requires_only_non_optional_sections(tmp_path: Path) -> None:
    ...
    completed = complete_weekly_review(
        service=service, state=state, now=SUNDAY, idempotency_key="complete"
    )
