...
def test_runtime_update_target_is_rejected_before_registry_or_target_read(
    tmp_path: Path,
    ...
    def fail_refresh(*args: object, **kwargs: object) -> object:
        ...
        pytest.fail("runtime target reached registry ingestion preflight")

    def fail_target_read(vault_root: Path, vault_path: str) -> object:
        target_reads.append(vault_path)
        pytest.fail("runtime target reached facade Markdown read")
    ...
    with pytest.raises(ToolError, match="Invalid LifeOS tool arguments"):
        server._tool_manager.get_tool("ingestion_update_wiki_section_proposal").fn(
            ...
            target_path="wiki/other/../runtime-node/export.md",
            heading="Summary",
            body="Replacement that must never inspect runtime bytes.",
        )
