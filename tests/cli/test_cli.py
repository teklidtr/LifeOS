...
def test_missing_database_exits_1_and_is_not_created(
    capsys: pytest.CaptureFixture[str], mock_config: Path
) -> None:
...
def test_existing_database_without_proposals_table_exits_1(
    capsys: pytest.CaptureFixture[str], mock_config: Path
) -> None:
    db_path = mock_config / "registry.db"
...
def test_corrupted_stored_status_exits_1(
    capsys: pytest.CaptureFixture[str], mock_config: Path
) -> None:
    db_path = mock_config / "registry.db"
    Registry(db_path).initialize()
    ...
    with Registry(db_path).connect() as conn:
        conn.execute(
            "INSERT INTO proposals (id, status, title, created_at, updated_at) VALUES ('id1', 'corrupt', 'Title', '2026-01-01', '2026-01-01')"
        )
...
def test_empty_result_prints_headers_only(
    capsys: pytest.CaptureFixture[str], mock_config: Path
) -> None:
    db_path = mock_config / "registry.db"
    Registry(db_path).initialize()
...
def test_list_proposals_output_format_and_filtering(
    capsys: pytest.CaptureFixture[str], mock_config: Path
) -> None:
    db_path = mock_config / "registry.db"
    Registry(db_path).initialize()
    ...
    with Registry(db_path).connect() as conn:
        conn.executemany(
            "INSERT INTO proposals (id, status, title, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
            [
...
def test_command_performs_no_scan_or_file_access(
    capsys: pytest.CaptureFixture[str], mock_config: Path
) -> None:
    db_path = mock_config / "registry.db"
    Registry(db_path).initialize()
    with Registry(db_path).connect() as conn:
        conn.execute(
            "INSERT INTO proposals (id, status, title, created_at, updated_at) VALUES ('p1', 'draft', 'T', '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z')"
        )

    with (
        patch("lifeos.cli.Path", return_value=mock_config / "lifeos.yml"),
        patch(
