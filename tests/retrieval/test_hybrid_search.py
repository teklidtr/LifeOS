...
def test_exact_hybrid_ranking_filters_links_and_duplicate_suppression(tmp_path: Path) -> None:
    ...
    filtered = retriever.search(
        RetrievalRequest("energy", scope=RetrievalScope(note_types=("journal",)))
    )
    assert filtered.results == ()
    linked = retriever.search(
        RetrievalRequest(
            "unrelated words",
            scope=RetrievalScope(
                paths=("wiki/mitochondria.md",), pinned_paths=("wiki/mitochondria.md",)
            ),
        )
    )
...
def test_semantic_paraphrase_and_irrelevant_similarity_do_not_hide_exact_evidence(
    tmp_path: Path,
) -> None:
    ...
    semantic = retriever.search(
        RetrievalRequest("cellular power production"), embedding_provider=provider
    )
...
def test_metadata_date_source_tag_folder_and_context_budget(tmp_path: Path) -> None:
    ...
    retriever = HybridRetriever(vault_root=vault, runtime_dir=runtime)
    scope = RetrievalScope(
        folders=("wiki",), tags=("biology",), sources=("lecture",), date_from="2026-02-01"
    )
    response = retriever.search(
        RetrievalRequest("energy organelles", scope=scope, context_budget=12)
    )
...
def test_no_provider_empty_scope_graph_hint_and_deterministic_ties(tmp_path: Path) -> None:
    ...
    empty = retriever.search(
        RetrievalRequest("nothing", scope=RetrievalScope(paths=("missing.md",)))
    )
    ...
    first = retriever.search(RetrievalRequest("concept"))
    second = retriever.search(RetrievalRequest("concept"))
...
def test_retrieval_evaluation_reports_separate_regression_metrics(tmp_path: Path) -> None:
    ...
    evaluation = evaluate_retrieval(
        (
            RetrievalFixture(
                "exact",
                RetrievalRequest("oxidative phosphorylation"),
                ("wiki/mitochondria.md",),
                k=3,
            ),
