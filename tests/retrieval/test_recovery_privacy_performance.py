...
def test_incompatible_and_corrupt_indexes_rebuild_without_touching_markdown(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(service.active_path) as connection:
        connection.execute("UPDATE meta SET value='99' WHERE key='schema_version'"); connection.commit()
...
def test_full_index_deletion_and_interrupted_rebuild_recovery_are_explicit(tmp_path: Path) -> None:
    vault = tmp_path / "vault"; vault.mkdir(); runtime = vault / ".lifeos"
    for index in range(8): write(vault, f"wiki/{index}.md", f"# Note {index}\n\nPassage {index}.")
    service = RetrievalIndexService(vault_root=vault, runtime_dir=runtime); service.rebuild()
    service.discard()
    assert service.recovery_plan().action == "full-rebuild"
    assert service.recover().processed == 8
    service.discard(); service.rebuild(stop_after=3, batch_size=1)
    plan = service.recovery_plan()
...
def test_large_vault_result_and_context_budgets_remain_bounded_and_stable(tmp_path: Path) -> None:
    ...
    for index in range(250):
        write(vault, f"wiki/{index:03d}.md", f"# Topic {index}\n\nShared retrieval phrase with detail {index}." + " x" * 100)
    service = RetrievalIndexService(vault_root=vault, runtime_dir=runtime); service.rebuild(batch_size=17)
    ...
    first = retriever.search(request); second = retriever.search(request)
    assert len(first.results) <= 7 and first.context_characters <= 333
    assert first.to_dict() == second.to_dict()


def test_embedding_batches_are_bounded_and_cancellation_preserves_active_index(tmp_path: Path) -> None:
    vault = tmp_path / "vault"; vault.mkdir(); runtime = vault / ".lifeos"
    for index in range(30): write(vault, f"wiki/{index}.md", f"# {index}\n\nText {index}.")
    service = RetrievalIndexService(vault_root=vault, runtime_dir=runtime); service.rebuild()
...
def test_protected_content_is_default_denied_and_external_disclosure_is_exact(tmp_path: Path) -> None:
    vault = tmp_path / "vault"; vault.mkdir(); runtime = vault / ".lifeos"
    ...
    service = RetrievalIndexService(vault_root=vault, runtime_dir=runtime, policy=policy); service.rebuild()
    retriever = HybridRetriever(vault_root=vault, runtime_dir=runtime, policy=policy)
    denied = retriever.search(RetrievalRequest("private evidence", scope=RetrievalScope(paths=("private/secret.md",), allow_protected=True)))
    assert denied.results == ()
    conversations = KnowledgeConversationService(vault_root=vault, runtime_dir=runtime, policy=policy)
    ...
    external = DeterministicAnswerProvider(
        GeneratedAnswer((GeneratedParagraph("Public claim.", (evidence_id,), "direct"),), "Grounded."),
        local_only=False,
    )
    saved = conversations.ask(
        artifact.relative_path, query="public evidence", expected_hash=artifact.content_hash,
