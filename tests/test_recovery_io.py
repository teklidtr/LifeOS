...
def test_recovery_artifact_write_preserves_corrupt_state_classification(
    tx_dir: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_recovery_artifact_write_preserves_conflict_classification(
    tx_dir: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_recovery_artifact_prepublication_failure_removes_temp(
    tx_dir: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from lifeos import _recovery_io
...
def test_recovery_artifact_temp_unlink_failure_preserves_final(
    tx_dir: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_recovery_artifact_temp_unlink_failure_is_reported(
...
def test_recovery_artifact_verification_failure_preserves_published_final(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tx_dir = tmp_path / "tx"
    tx_dir.mkdir()
...
def test_recovery_artifact_final_verification_reopens_final_name(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tx_dir = tmp_path / "tx"
    tx_dir.mkdir()
...
def test_recovery_artifact_final_verification_checks_hash(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tx_dir = tmp_path / "tx"
    tx_dir.mkdir()
...
def test_recovery_artifact_final_verification_checks_size(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tx_dir = tmp_path / "tx"
    tx_dir.mkdir()
...
def test_recovery_artifact_final_verification_checks_mode(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tx_dir = tmp_path / "tx"
    tx_dir.mkdir()
...
def test_remove_installed_creation_does_not_parse_transaction_error_message(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    target_dir = tmp_path / "target"
    target_dir.mkdir()
    ...
    try:
        ...
        with pytest.raises(RecoveryIOUnavailableError):
            remove_installed_creation(
                target_name="file.txt",
                target_parent=parent,
                expected_installed_hash="sha256:" + "0" * 64,
                expected_installed_mode=0o644,
            )
    finally:
        os.close(fd)
...
def test_recovery_artifact_read_verifies_size(tmp_path: Path) -> None:
    ...
    artifact = RecoveryArtifact(
        relative_path="staged/file.txt", content_hash=f"sha256:{h}", size=999, mode=0o644
    )
    with pytest.raises(RecoveryIOCorruptStateError):
        read_verified_recovery_artifact(transaction_dir=tx_dir, artifact=artifact)
...
def test_recovery_artifact_read_verifies_mode(tmp_path: Path) -> None:
    ...
    h = hashlib.sha256(b"hello").hexdigest()
    artifact = RecoveryArtifact(
        relative_path="staged/file.txt", content_hash=f"sha256:{h}", size=5, mode=0o644
    )
    with pytest.raises(RecoveryIOCorruptStateError):
        read_verified_recovery_artifact(transaction_dir=tx_dir, artifact=artifact)
...
def test_recovery_artifact_write_failure_removes_temporary_file(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    tx_dir = tmp_path / "tx"
    tx_dir.mkdir()
...
def test_recovery_artifact_publication_failure_preserves_existing_final_file(
    tmp_path: Path,
) -> None:
    tx_dir = tmp_path / "tx"
    tx_dir.mkdir()
...
def test_prepare_canonical_staging_from_artifact_works_across_filesystems(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    target_dir = tmp_path / "target"
    target_dir.mkdir()
    ...
    try:
        ...
        staged = prepare_canonical_staging_from_artifact(
            transaction_dir=tmp_path / "tx",
            artifact=artifact,
            target_name="file.txt",
            target_parent=parent,
            intended_mode=artifact.mode,
        )
...
def test_prepare_canonical_staging_from_artifact_uses_target_local_parent(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    target_dir = tmp_path / "target"
    target_dir.mkdir()
    ...
    try:
        ...
        with pytest.raises(RecoveryIOConflictError):
            remove_installed_creation(
                target_name="file.txt",
                target_parent=parent,
                expected_installed_hash="sha256:2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824",
                expected_installed_mode=0o644,
            )
    finally:
        os.close(fd)
...
def test_restore_canonical_from_backup_verifies_installed_state(tmp_path: Path) -> None:
    ...
    try:
        ...
        parent = ParentDescriptor(fd=fd, dev=st.st_dev, ino=st.st_ino, path=str(target_dir))
        backup_artifact = RecoveryArtifact(
            relative_path="staged/backup.txt",
            content_hash="sha256:2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824",
            size=5,
            mode=0o644,
        )
...
def test_restore_canonical_from_backup_verifies_backup_hash(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    target_dir = tmp_path / "target"
    target_dir.mkdir()
...
def test_restore_canonical_from_backup_verifies_backup_mode(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    target_dir = tmp_path / "target"
    target_dir.mkdir()
...
def test_restore_canonical_from_backup_uses_target_local_atomic_publish(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    target_dir = tmp_path / "target"
    target_dir.mkdir()
...
def test_restore_canonical_from_backup_verifies_restored_state(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    target_dir = tmp_path / "target"
    target_dir.mkdir()
...
def test_restore_canonical_from_backup_restores_real_bytes_and_mode(tmp_path: Path) -> None:
    ...
    try:
        ...
        restore_canonical_from_backup(
            transaction_dir=tx_dir,
            backup=RecoveryArtifact(
                relative_path="backups/backup.txt.bak",
                content_hash=f"sha256:{b_hash}",
                size=len(b"original backup content"),
                mode=0o644,
            ),
            target_name="file.txt",
            target_parent=parent,
