...
def test_empty_dashboard_distinguishes_missing_from_zero(tmp_path: Path) -> None:
    ...
    result = build_today_dashboard(
        ...
        inputs=TodayInputs(date(2026, 7, 16), available_minutes=0, study_minutes=0),
    )
...
def test_planner_failure_does_not_erase_other_cards(tmp_path: Path) -> None:
    ...
    result = build_today_dashboard(
        vault_root=vault, runtime_dir=tmp_path / "runtime", inputs=TodayInputs(date(2026, 7, 16))
    )
...
""",
    ...
    inputs = TodayInputs(
        date(2026, 7, 16), available_minutes=30, energy="medium", motivation="medium"
    )
    first = build_today_dashboard(vault_root=vault, runtime_dir=tmp_path / "runtime", inputs=inputs)
    second = build_today_dashboard(
        vault_root=vault, runtime_dir=tmp_path / "runtime", inputs=inputs
    )
    assert first == second
