...
def test_multiple_attempts_and_rich_evidence_rebuild(tmp_path: Path) -> None:
    ...
    app.record_task_outcome(
        TaskOutcomeRequest(
            ...
            reason="scope",
            note="Outlined",
            started_at="2026-07-15T23:50:00+03:00",
            ended_at="2026-07-16T00:10:00+03:00",
        )
    )
    second_hash = content_hash(plan.read_text())
    app.record_task_outcome(
        TaskOutcomeRequest(
            "event-2", "plans/p.md", "t", "done", date(2026, 7, 16), second_hash, actual_minutes=15
        )
    )
...
def test_terminal_transition_fails_without_history_change(tmp_path: Path) -> None:
    ...
    app.record_task_outcome(
        TaskOutcomeRequest(
            "event-1", "plans/p.md", "t", "done", date(2026, 7, 16), content_hash(plan.read_text())
        )
    )
    ...
    with pytest.raises(DailyInteractionError) as caught:
        app.record_task_outcome(
            TaskOutcomeRequest(
                "event-2", "plans/p.md", "t", "skipped", date(2026, 7, 17), content_hash(before)
            )
        )
