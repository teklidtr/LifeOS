...
def test_runtime_target_is_rejected_before_base_preflight_reads_anything(
    tmp_path: Path,
    ...
    documents = build_wiki_proposal(
        content=WikiProposalContent(
            ...
            body="Runtime bytes must never participate in ordinary preflight.",
            generator=generator,
        ),
        source=SourceSnapshot(path="raw/source.md", content_hash="sha256:" + "1" * 64),
        target_path="wiki/runtime-node/new.md",
        proposal_id="prop-20260825T073000Z-1234abcd",
    ...
    def fail_base_preflight(*args: object, **kwargs: object) -> object:
        pytest.fail("runtime target reached ordinary preflight")
