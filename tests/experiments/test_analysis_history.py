...
from lifeos.experiments import (
    ExperimentArtifactService, ExperimentPhase, ExperimentProtocol, MeasureDefinition,
    analyze_experiment, build_visual_model, compare_experiments, create_observation,
    load_experiment_index, rebuild_experiment_index, record_conclusion, save_analysis,
)
...
def protocol() -> ExperimentProtocol:
    return ExperimentProtocol(
        question="Does schedule B relate to focus?", hypothesis="B has higher focus", rationale="compare",
        intervention="Use schedule B", constants=("same material",), comparison="schedule A", baseline_requirements="3 days",
        outcome_measures=(
            MeasureDefinition("focus", "Focus", "rating", "primary", "daily", valid_min=1, valid_max=10),
            MeasureDefinition("adherence", "Followed schedule", "completion", "adherence", "daily", aggregation="rate"),
            MeasureDefinition("notes", "Context notes", "qualitative", "contextual", "daily", aggregation="none"),
        ),
        phases=(
        ...
        ), adherence_expectation="all days", confounders=("sleep",), risks=(), stop_rules=(),
...
def seeded(tmp_path: Path):
    ...
    item = api.create(title="Schedules", description="", category="study", protocol=protocol(), now=NOW)
    ...
    for index, (phase, value) in enumerate(values):
        ...
        obs = create_observation(item.metadata, measure_id="focus", phase_id=phase, observed_at=NOW + timedelta(days=index), state=state, value=value, note="missing" if value is None else "")
        item = api.append_observation(item.path, obs, expected_hash=item.content_hash, now=NOW + timedelta(days=index))
    for index, phase in enumerate(("a", "a", "b", "b")):
        obs = create_observation(item.metadata, measure_id="adherence", phase_id=phase, observed_at=NOW + timedelta(days=index), state="measured", value=index != 1)
        item = api.append_observation(item.path, obs, expected_hash=item.content_hash, now=NOW + timedelta(days=index))
    note = create_observation(item.metadata, measure_id="notes", phase_id="b", observed_at=NOW, state="measured", value="Travel day changed the context.", context=("possible confounder",))
...
def test_analysis_preserves_missingness_raw_ids_and_noncausal_limitations(tmp_path: Path) -> None:
    ...
    concluded = record_conclusion(api, saved, conclusion="mixed", notes="Focus improved but travel confounded the comparison.", follow_up_decisions=("repeat",), expected_hash=saved.content_hash, now=NOW)
    assert concluded.metadata.conclusion == "mixed"


def test_insufficient_analysis_and_visual_fallback(tmp_path: Path) -> None:
    api = ExperimentArtifactService(vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos")
    item = api.create(title="Sparse", description="", category="study", protocol=protocol(), now=NOW)
    skipped = create_observation(item.metadata, measure_id="focus", phase_id="a", observed_at=NOW, state="skipped", note="forgot")
...
def test_rebuild_handles_rename_runtime_deletion_duplicates_interrupt_and_large_history(tmp_path: Path) -> None:
    runtime = tmp_path / ".lifeos"
    api = ExperimentArtifactService(vault_root=tmp_path, runtime_dir=runtime)
    created = []
    for index in range(105):
        created.append(api.create(title=f"Experiment {index}", description="", category="study", protocol=protocol(), now=NOW + timedelta(seconds=index)))
    interrupted = rebuild_experiment_index(vault_root=tmp_path, runtime_dir=runtime, batch_size=10, interrupt_after=20)
...
def test_lineage_and_incompatible_comparison_warning(tmp_path: Path) -> None:
    api = ExperimentArtifactService(vault_root=tmp_path, runtime_dir=tmp_path / ".lifeos")
    first = api.create(title="First", description="", category="study", protocol=protocol(), now=NOW)
    ...
    comparison = compare_experiments(report.entries, first.metadata.experiment_id, repeat.metadata.experiment_id)
