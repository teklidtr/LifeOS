...
def observation(
    ...
    *,
    ...
    motivation: str | None = "low",
    mode: str = "writing",
    planned: int = 30,
    started: str | None = "2026-07-15T09:00:00+03:00",
    ...
    return FeedbackObservation(
        ...
        blocked,
        outcome,
        1.0 if outcome == "done" else 0.5 if outcome == "partial" else 0.0,
        planned,
        planned,
        energy,
        None,
...
def test_energy_and_motivation_remain_distinct() -> None:
    items = tuple(
        observation(i, energy="high", motivation="low", outcome="done") for i in range(8)
    ) + tuple(
        observation(20 + i, energy="low", motivation="high", outcome="skipped") for i in range(8)
    )
    result = summarize_capacity_fit(
        observations=items,
        task_id="task",
        current_energy="high",
...
def test_missing_and_disabled_dimensions_do_not_count_negative() -> None:
    ...
    result = summarize_capacity_fit(
        ...
        current_motivation="medium",
        mode="writing",
        declared_minutes=30,
        as_of=AS_OF,
        disabled_dimensions=("mode",),
    )
    by_name = {item.name: item for item in result.dimensions}
...
def test_minimum_freshness_conflict_and_shuffle_boundaries() -> None:
    ...
    second = summarize_capacity_fit(
        observations=reversed(mixed),
        task_id="task",
        current_energy="high",
        current_motivation="low",
        mode="writing",
