...
def test_all_outcomes_and_missing_values_remain_distinct(tmp_path: Path) -> None:
    ...
    assert [item.outcome for item in observations] == ["partial", "skipped", "deferred", "cancelled", "unaccounted"]
...
def test_duplicate_conflict_invalid_chronology_and_unsupported_schema_are_diagnostics(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    write_plan(vault, """  - event_id: duplicate\n    task_id: write-note\n    outcome: done\n    date: 2026-07-10\n  - event_id: duplicate\n    task_id: write-note\n    outcome: done\n    date: 2026-07-11\n  - event_id: bad-time\n    task_id: write-note\n    outcome: done\n    date: 2026-07-12\n    started_at: 2026-07-12T11:00:00+03:00\n    ended_at: 2026-07-12T10:00:00+03:00\n  - event_id: future\n    schema_version: 9\n    task_id: write-note\n    outcome: done\n    date: 2026-07-13""")
    dataset = build_evidence_dataset(vault)
...
def test_orphaned_task_is_retained_with_warning(tmp_path: Path) -> None:
    vault = tmp_path / "vault"
    write_plan(vault, """  - event_id: old\n    task_id: removed-task\n    task_title: Old title\n    mode: reading\n    planned_minutes: 20\n    outcome: done\n    date: 2026-07-10\n    actual_minutes: 18""")
    dataset = build_evidence_dataset(vault)
    assert dataset.observations[0].task_id == "removed-task"
    assert any(item.code == "orphaned_task_reference" and item.severity == "warning" for item in dataset.diagnostics)
