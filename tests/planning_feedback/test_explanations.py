...
def action(task: str, duration: int, *, energy: str = "medium", mode: str = "writing") -> PlanningAction:
    return PlanningAction(task, task.title(), "todo", duration, energy, "medium", mode, "goal", "plan", None, (), "plans/p.md")  # type: ignore[arg-type]


def event(index: int, task: str, actual: int) -> FeedbackObservation:
    return FeedbackObservation(1, f"o{index}", f"e{index}", "plans/p.md", "h", index, AS_OF - timedelta(days=index), "plan", "goal", task, task, "unspecified", "writing", "medium", "medium", False, "done", 1.0, 30, actual, "medium", None, "medium", None, None, None, (), False)
...
def test_rejected_candidate_gets_time_energy_mode_and_evidence_counterfactuals() -> None:
    ...
    result = build_adaptive_menu(actions=actions, observations=(), as_of=AS_OF, available_minutes=30, energy="low", motivation="medium", adaptive_mode="active")
...
def test_disabled_low_confidence_signals_are_named_without_private_text() -> None:
    actions = (action("a", 30),)
    result = build_adaptive_menu(actions=actions, observations=(event(1, "a", 50),), as_of=AS_OF, available_minutes=30, energy="medium", motivation="medium", adaptive_mode="shadow", disabled_dimensions=("energy", "motivation"))
    explanation = explain_adaptive_result(result=result, actions=actions, task_id="a")
...
def test_deterministic_order_and_unknown_task() -> None:
    actions = (action("a", 30), action("b", 30))
    history = tuple(event(i, "a", 45) for i in range(6))
    first_result = build_adaptive_menu(actions=actions, observations=history, as_of=AS_OF, available_minutes=30, energy="medium", motivation="medium", adaptive_mode="active")
    second_result = build_adaptive_menu(actions=tuple(reversed(actions)), observations=reversed(history), as_of=AS_OF, available_minutes=30, energy="medium", motivation="medium", adaptive_mode="active")
    assert explain_adaptive_result(result=first_result, actions=actions, task_id="a") == explain_adaptive_result(result=second_result, actions=actions, task_id="a")
