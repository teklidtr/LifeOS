...
def test_complete_desktop_daily_loop_without_cli(tmp_path: Path) -> None:
    ...
    client = ReferenceBridgeClient(
        BridgeApplication(vault_root=vault, runtime_dir=runtime, actor_id="user")
    )
    ...
    captured = client.call(
        ...
        kind="thought",
        title="Captured in Obsidian",
        content="No terminal used.",
    )
    ...
    client.call(
        ...
        plan_path="plans/fixture.md",
        task_id="fixture-task",
        outcome="done",
        day="2026-07-16",
        expected_hash=content_hash(plan.read_text()),
        actual_minutes=18,
    )
    session = client.call(
        "study.session.start",
...
def test_incompatible_protocol_fails_before_mutation(tmp_path: Path) -> None:
    ...
    vault.mkdir()
    client = ReferenceBridgeClient(
        BridgeApplication(vault_root=vault, runtime_dir=tmp_path / "runtime", actor_id="user")
    )
    try:
