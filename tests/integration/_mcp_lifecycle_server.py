...
from lifeos.facade.authorization import (
    ConsequentialAuthorizer, AuthorizedPrincipal, ConsequentialAction,
    ConsequentialAuthorizationRequest, AuthorizationDeniedError
)
...
class RecordingAuthorizer(ConsequentialAuthorizer):
    ...
    def authorize(self, request: ConsequentialAuthorizationRequest) -> AuthorizedPrincipal:
        ...
        if request.action in self.deny_actions:
            raise AuthorizationDeniedError(f"Action {request.action.value} denied by test authorizer")
...
def main() -> None:
    ...
    try:
        ...
        authorizer = RecordingAuthorizer(
            ...
            log_path=Path(args.authorization_log),
