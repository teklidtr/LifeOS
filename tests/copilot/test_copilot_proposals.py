...
def _request(goal, snapshot, option, decomposition, **changes):
    values = dict(
        ...
        expected_session_revision=snapshot.envelope.session.source_revision,
        goal_path=goal.path,
        expected_goal_hash=goal.content_hash,
        plan_id="plan-cell-new",
        plan_path="plans/cell-new.md",
        plan_title="Edited cell plan",
...
def test_new_plan_goal_link_item_edits_and_exclusion_are_proposal_only(tmp_path: Path) -> None:
    ...
    request = _request(
        ...
        option,
        decomposition,
        milestone_edits={milestone_id: {"title": "Edited milestone"}},
        action_edits={action_id: {"duration": 45, "title": "Edited next action"}},
    )
    ...
    loaded = load_proposal_directory(
        tmp_path / result.proposal_path, proposals_root=tmp_path / "proposals"
    )
...
def test_selected_exclusion_and_unrelated_goal_body_are_preserved_after_apply(
    tmp_path: Path,
) -> None:
    _goal(tmp_path)
    goal, sessions, snapshot, option, decomposition, index = _draft(tmp_path)
...
def test_explicit_supersession_uses_exact_hash_and_preserves_human_body(tmp_path: Path) -> None:
    ...
    request = _request(
        ...
        option,
        decomposition,
        conflict_edits=(ConflictPlanEdit("plans/old.md", "supersede"),),
    )
    result = create_copilot_plan_proposal(
        vault_root=tmp_path,
        option=option,
        ...
        actor_id="tester",
        session_service=sessions,
        now=datetime(2026, 7, 16, 0, 0, 2, tzinfo=timezone.utc),
    )
...
def test_bridge_creates_reviewable_proposal_without_applying(tmp_path: Path) -> None:
    ...
    result = app.dispatch(
        "copilot.proposal.create",
        {
            ...
            "plan_id": "plan-cell-bridge",
            "plan_path": "plans/cell-bridge.md",
            "plan_title": "Bridge cell plan",
            "desired_outcome": "Explain six chapters",
