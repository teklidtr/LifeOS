...
def _option(outcome: str = "Explain six cell biology chapters") -> PlanOption:
    return PlanOption(
        ...
        "option-cell",
        "Cell plan",
        "Study in a bounded wave",
        outcome,
        ("Protect exercise and rest",),
        (),
        ("Six notes",),
        ("Pace risk",),
        None,
...
def _action(
    ...
    *,
    ...
    return ActionSuggestion(
        title=f"Complete bounded work for {task_id}",
        task_id=task_id,
        milestone_id="milestone-cell",
        duration=minutes,
        energy="medium",
        motivation="medium",
        mode="study",
        ...
        rationale="Visible first step",
        verification="A visible note",
...
def _plan(
    plan_id: str, *, outcome: str = "Different outcome", tasks: tuple[NearTermAction, ...] = ()
) -> PlanRecord:
    return PlanRecord(
        1,
        plan_id,
...
def test_comfortable_marginal_overload_and_unknown_capacity() -> None:
    option = _option()
    decomp = decompose_plan_option(
        option=option, horizon="months", adapter=Adapter((_action("task-new", 60),))
    )
    protected = (RecurringWorkload("run", "Running", 60, kind="exercise"),)
    comfortable = check_portfolio_capacity(
        option=option,
        decomposition=decomp,
        index=_index(),
        as_of=date(2026, 7, 16),
        available_minutes=600,
        recurring_workloads=protected,
    )
    marginal = check_portfolio_capacity(
        option=option,
        decomposition=decomp,
        index=_index(),
        as_of=date(2026, 7, 16),
    ...
    overload = check_portfolio_capacity(
        option=option,
        decomposition=decomp,
        index=_index(),
        as_of=date(2026, 7, 16),
    ...
    unknown = check_portfolio_capacity(
        option=option,
        decomposition=decomp,
        index=_index(),
        as_of=date(2026, 7, 16),
    ...
    assert [comfortable.fit, marginal.fit, overload.fit, unknown.fit] == [
        "comfortable",
        "marginal",
        "overload",
        "unknown",
...
def test_missing_duration_is_unknown_not_zero_and_adaptive_view_stays_separate() -> None:
    option = _option()
    decomp = decompose_plan_option(
        option=option, horizon="months", adapter=Adapter((_action("task-new", None),))
    )
    report = check_portfolio_capacity(
        option=option,
        decomposition=decomp,
        index=_index(),
        as_of=date(2026, 7, 16),
        available_minutes=100,
        adaptive_durations={"task-new": 120},
    )
...
def test_due_prerequisite_duplicate_and_many_plan_findings_are_inspectable() -> None:
    ...
    decomp = decompose_plan_option(
        ...
        explicit_deadlines={"task-a": due, "task-b": due},
        existing_task_ids=("task-prereq",),
    )
    existing = NearTermAction("task-existing", "Existing deadline", duration=80, due=due)
    plans = (
        _plan("plan-duplicate", outcome="Explain six cell biology chapters", tasks=(existing,)),
    ) + tuple(_plan(f"plan-{i}") for i in range(2, 8))
    report = check_portfolio_capacity(
        option=option,
        decomposition=decomp,
        index=_index(plans),
    ...
    assert all(
        item.possible_adjustments for item in report.findings if item.severity != "information"
    )
...
def test_stable_under_shuffled_inputs() -> None:
    ...
    workloads = (
        RecurringWorkload("b", "Rest", 30, kind="rest"),
        RecurringWorkload("a", "Hobby", 30, kind="hobby"),
    )
    first = check_portfolio_capacity(
        option=option,
        decomposition=decomp,
        index=_index((_plan("plan-b"), _plan("plan-a"))),
        as_of=date(2026, 7, 16),
        available_minutes=500,
        recurring_workloads=workloads,
    )
    second = check_portfolio_capacity(
        option=option,
        decomposition=decomp,
        index=_index((_plan("plan-a"), _plan("plan-b"))),
        as_of=date(2026, 7, 16),
        available_minutes=500,
        recurring_workloads=tuple(reversed(workloads)),
    )
...
def test_bridge_exposes_read_only_capacity_check(tmp_path: Path) -> None:
    ...
    result = app.dispatch(
        "copilot.capacity.check",
        {
            ...
            "recurring_workloads": [
                {"workload_id": "run", "title": "Running", "minutes": 60, "kind": "exercise"}
            ],
        },
    )
