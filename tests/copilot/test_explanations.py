...
def _context(*, stale: bool = False, omitted: bool = False) -> PlanningContextPack:
    ...
    return PlanningContextPack(
        ...
        (ContextOmission("journal/private.md", "explicitly-excluded", "Excluded"),)
        if omitted
        else (),
        4,
        False,
...
def _option(
    option_id: str = "option-cell",
    *,
    source_ref: str = "goals/cell.md",
    assumption_kind: str = "canonical-note",
) -> PlanOption:
    return PlanOption(
        1,
        option_id,
        "Cell plan",
        "Use one bounded study wave",
        "Explain six chapters",
        ("Do not finish the whole book",),
        (
            PlanAssumption(
                f"assumption-{option_id}",
                "Four hours remain available",
                assumption_kind,
                source_ref if assumption_kind != "agent-assumption" else None,
                "medium",
            ),
        ),
        ("Six notes",),
        ...
        (
            Milestone(
                f"milestone-{option_id}", "Build foundation", "Explain two chapters", wave="current"
            ),
        ),
        ("Narrow scope",),
        ("Is four hours realistic?",),
        (source_ref,),
...
def _bundle(option: PlanOption, minutes: int | None = 300):
    ...
    capacity = check_portfolio_capacity(
        ...
        recurring_workloads=(RecurringWorkload("rest", "Rest", 60, "rest"),),
    )
...
def test_explanation_distinguishes_sources_assumptions_stale_and_omissions() -> None:
    ...
    result = explain_plan_option(
        option=option,
        decomposition=decomp,
        capacity=capacity,
        context=_context(stale=True, omitted=True),
    )
...
def test_invalid_explanation_reference_is_rejected() -> None:
    ...
    with pytest.raises(ExplanationError, match="invalid explanation source"):
        explain_plan_option(
            option=option, decomposition=decomp, capacity=capacity, context=_context()
        )
...
def test_comparison_uses_explicit_dimensions_and_no_winner() -> None:
    ...
    da, ca = _bundle(a, 400)
    db, cb = _bundle(b, 100)
    result = compare_plan_options(
        options=(b, a),
        decompositions={a.option_id: da, b.option_id: db},
        capacity_reports={a.option_id: ca, b.option_id: cb},
    )
...
def test_counterfactual_is_recomputed_not_improvised() -> None:
    ...
    result = recompute_capacity_counterfactual(
        option=option,
        decomposition=decomp,
        index=CopilotIndex((), (), ()),
        before=before,
...
def test_stable_serialization() -> None:
    option = _option()
    decomp, capacity = _bundle(option)
    first = explain_plan_option(
        option=option, decomposition=decomp, capacity=capacity, context=_context()
    ).to_dict()
    second = explain_plan_option(
        option=option, decomposition=decomp, capacity=capacity, context=_context()
    ).to_dict()
    assert first == second
...
def test_bridge_exposes_explanation_and_counterfactual(tmp_path: Path) -> None:
    ...
    counter = app.dispatch(
        "copilot.counterfactual",
        {
            "session_id": "session-cell",
            "option_id": option_id,
            "as_of": "2026-07-16",
            "before_available_minutes": 300,
            "available_minutes": 30,
        },
    )
    comparison = app.dispatch(
        "copilot.compare",
        {
            "session_id": "session-cell",
